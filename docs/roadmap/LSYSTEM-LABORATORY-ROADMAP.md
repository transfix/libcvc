...
```json
{
  ...
  "validation": {
    ...
    "outdoor": { "policy": "forced-bridges",
                 ...
                 "world_bridges_expected": 2, "world_bridges_verified": 2,
                 "world_narrowest_bridge_m": 16.5, "bridge_min_required_m": 16.0,
                 "bridges_in_window": 1, "narrowest_bridge_in_window_m": 17.4,
...
Four fields in `validation.outdoor` deserve a sentence each. **The `world_*` fields are `validate_world`'s, not this window's** — they are the authoritative bridge numbers, measured over the whole landmass (widths on a 0.5 m strip crop, §7.8) and copied in verbatim; `world_narrowest_bridge_m` is `null` exactly when `world_bridges_expected` is 0, which is the `wasm` / single-island case and not a missing measurement. **`bridges_in_window` and `narrowest_bridge_in_window_m` are the window's own view and are diagnostics** — a window that crops away every isthmus reports `0` / `null` and still passes, because the acceptance clauses read only the `world_*` fields. **`world_components` is D9's binding criterion** and is a property of the *world*, not of this window; it is copied in so a bundle can be audited without regenerating the terrain. **`channels_crossed` is no longer a failure signal** under `forced-bridges` — a window that straddles a strait is fine precisely because a bridge exists somewhere in the landmass; the field is retained as a difficulty-relevant statistic (an episode that crosses a channel has to find the causeway) and is one of the raw numbers §10.3 keeps so a better curriculum label can be computed retroactively.

**`meta` values are in the NORMALIZED frame, not metres.** This is a correction to revision 1, which wrote `rr: 6.0, d_hat: 12.0, vmax: 8.0, dt: 0.1, nsub: 1` — metre-flavoured numbers that `SdfNavigator` consumes in the same normalized frame as `VEHICLE_DEFAULTS["L"] = 0.035` (≈ 0.7 m wheelbase at `scale = 0.05`). A bundle written with `rr = 6.0` would give the vehicle a repulsion radius comparable to the entire half-region (`region · scale = 128 × 0.05 = 6.4`) and would be unusable. The canonical values, verified in `grl_snam/tools/austin.py:85-95` and `grl_snam/tools/material_demo.py:31-44`, are `scale = 0.05, rr = 0.15, d_hat = 0.35, dt = 0.06, nsub = 2, vmax = 0.9` — i.e. `rr/scale = 3.0 m` of vehicle radius and `d_hat/scale = 7.0 m` of geometry-SDF reach. **Revision 2 wrote "≈ 3 m wheelbase" above; that was the *vehicle-radius* figure (`rr/scale`) copied onto `L`.** `L` is normalized like every other length here, so `L/scale = 0.035/0.05 = 0.7 m` — and the ratio that matters is frame-independent: `rr/L = 4.3`, i.e. the collision disc is **4.3 wheelbases** in radius, against roughly 0.8 for a disc that merely circumscribes a car body. That is deliberate margin, not a unit error, but it is margin: quote `rr/L`, never a metre figure for `L` alone, or the next reader will infer a family car and mistake a ~5× footprint inflation for a tight fit. Note also that `meta["d_hat"]` (**normalized**, the geometry barrier) and `MaterialParams.d_hat_sdf_m` (**12.0 metres**, the *material* barrier) are two different quantities; revision 1 copied the latter into the former.
...
struct outdoor_gate_report {
  ...
  // ---- WINDOW SCOPE. Filled by validate_outdoor on the 0.5 m EXPORT lattice. ----
  ...
  int    bridges_in_window = 0;        // DIAGNOSTIC. Isthmus centrelines this crop
                                       // contains. A window holding none reports 0,
                                       // and that is NOT a failure.
  std::optional<double>
         narrowest_bridge_in_window_m; // DIAGNOSTIC, never an acceptance term.
                                       // nullopt when bridges_in_window == 0.
  int    bridges_forced = 0, resamples = 0;   // repairs THIS window needed

  // ---- WORLD SCOPE. Filled by validate_world; COPIED into every window report. ----
  int    world_components = 0;         // D9's binding criterion
  int    world_bridges_expected = 0;   // N-1 under forced-bridges; 0 otherwise
  int    world_bridges_verified = 0;   // how many survived to the raster
  std::optional<double>
         world_narrowest_bridge_m;     // AUTHORITATIVE width. MEASURED on a 0.5 m strip
                                       // crop of the exported bytes, never assumed.
                                       // nullopt IFF world_bridges_expected == 0,
                                       // i.e. exactly when N == 1 or force_bridges=false.
...
// Fills the WINDOW-SCOPE fields ONLY; the caller copies the world-scope fields in from
// the validate_world verdict recorded when the world was generated.
...
// Rasterises the full extent on the COARSE CONNECTIVITY LATTICE (2.0 m) and labels it.
// AUTHORITATIVE for world_components, world_bridges_expected / world_bridges_verified
// and world_narrowest_bridge_m.
outdoor_gate_report validate_world(const world_model&, const archipelago_spec&,
                                   double lattice_m = 2.0, double inflate_m = 6.0);
```

**Two producers, one authority.** The bridge numbers had two producers at two scopes on two lattices — `validate_world` over the whole extent at 2.0 m, `validate_outdoor` over one window at 0.5 m — and nothing said which one a manifest field or an acceptance row meant. Pinned, in one direction: **`validate_world` is authoritative.** It is the only producer that knows `N`, so it is the only one that can compute `bridges_expected = N − 1`; it counts every isthmus in the landmass rather than the subset a rectangle happens to contain; and it re-measures each verified bridge on a **0.5 m strip crop** around its centreline, so the authoritative width is a fine-lattice measurement even though the labelling that found the bridge was coarse. `validate_outdoor` reports the **window's** view — how many isthmus centrelines this crop contains, and the narrowest of *those* — as a **diagnostic only**, never as an acceptance term, because a legitimate 256 m window will usually contain no bridge at all and must not be failed for it. The field names now carry their scope (`world_bridges_expected` / `world_bridges_verified` / `world_narrowest_bridge_m` versus `bridges_in_window` / `narrowest_bridge_in_window_m`) so the two can never be confused in a struct, a manifest, a test or a UI readout, and each window report carries the world numbers verbatim so a written bundle is auditable without regenerating the terrain.

**Why the world check runs on a 2 m lattice and why that is still sound.** The full `large` extent at the 0.5 m export lattice is 16 384² = 268 M cells — an EDT and a labelling pass nobody is going to run per world. At 2.0 m that same extent is 4 096² = **16.8 M** cells; `standard`, whose extent is **4 096 m** (§4.2, §4.3), is **2 048** cells across = **4.2 M**, and `wasm`, at 2 048 m, is **1 024** across = **1.05 M**. That is one EDT + one union-find in ≈ 150 ms (`large`) / ≈ 40 ms (`standard`) / ≈ 12 ms (wasm), on a transient buffer of 5 B/cell (1 B occupancy + a 32-bit EDT scratch) — 84 MB / 21 MB / 5 MB — freed the moment the verdict is recorded. *(Corrected: this paragraph previously read "4 096 cells across for `standard`", which is the `large` figure; `standard` is half that extent and therefore a quarter of the cells. The §11.2 time and memory rows were already derived from the correct 4.2 M and do not move — §11.2 now shows the cell counts inline so the arithmetic is checkable at the point of use.)*

**The coarsening rule, and the one direction it is conservative in.** Obstacles are inflated by `inflate_m` on the 0.5 m field **first**; the 2 m lattice is then the **AND-coarsening** of that inflated-free set — a coarse cell is free only if *all* of its fine cells are free. That is an **erosion**, so the coarse free set is a **subset** of the fine one, and coarse 4-adjacency lifts to a fine 4-connected path. The consequence that matters is about *crossings*, not about counts: **every crossing the coarse lattice finds is a real crossing on the fine bytes, so the coarse pass can never report CONNECTED where the fine lattice is severed.** That is the conservative direction for a connectivity claim, and it is the only one the gate needs — a false *rejection* costs one rebuilt bridge, a false *acceptance* ships an unsolvable corpus. Errors therefore fall the safe way: a 16 m isthmus is ≥ 8 coarse cells wide and survives comfortably, while a 2 m sliver that only exists at 0.5 m is erased, the coarse pass sees a severance the fine bytes do not have, and we rebuild a bridge we might not have needed. We would rather rebuild a bridge than certify a sliver.

**What the subset relation does *not* give — corrected, because the argument and its test disagreed.** Revision 2.1 wrote that a coarse `components == 1` *implies* a fine `components == 1`, and defended it with a test asserting that "the 2 m labelling never reports fewer components than the 0.5 m labelling". Neither is sound, for the same reason: an erosion does not only *sever* components, it can *delete* one outright. A fine component thinner than a coarse cell — a stranded sliver islet — vanishes from the coarse lattice entirely, so the coarse **count** can be *lower* than the fine count while the coarse labelling is still perfectly sound about every crossing. The count is not monotone; the crossing property is. The gate rests on the crossing property, and the residue is bounded and harmless: labelling runs on the *inflated*-free set, so a fine feature the 2 m erosion can erase is under 2 m across in the inflated-free set, i.e. a free corridor or patch under `2 + 2·inflate_m = 14 m` wide — already beneath the `w_min = 16.0 m` floor at which the gate would accept a crossing at all. Anything erased is therefore an unreachable pocket, endpoints are drawn from **one** component (step 6), and every *exported* window is labelled again at the 0.5 m export lattice and must itself report `components == 1`. §12.3's conservatism test is restated to assert the crossing property, plus the count comparison in the only form that is provable.

Each verified bridge's width is re-measured on a **0.5 m strip crop** around its centreline, so `world_narrowest_bridge_m` is always a fine-lattice measurement.
...
5. Record component count, largest_component_fraction, and the per-pair channel /
   bridge widths of section 4.3a.4. MEASURE narrowest_bridge_in_window_m from the
   land EDT over the isthmuses THIS window contains -- a DIAGNOSTIC; the
   authoritative world_narrowest_bridge_m comes from validate_world. Never copy
   the half-width the constructor asked for. Both are nullopt when there is no
   bridge in scope, never 0.0.
...
|---|---|
| `forced-bridges` (**v1 default — D9**) | `world_components == 1` **and** window `components == 1` after repair, **and** `world_bridges_verified == world_bridges_expected == N − 1`, **and** — *only when `world_bridges_expected > 0`* — `world_narrowest_bridge_m ≥ w_min = max(2·agent_radius_m + 1.0, 2·inflate_m + 4.0)` (**= 16.0 m** at the shipped `inflate_m = 6.0`, for both agent presets — §4.3a.4). At `N = 1` the width clause is **vacuous**; see below. The window-scope `bridges_in_window` / `narrowest_bridge_in_window_m` are **diagnostics and appear in no clause.** |
...
| `amphibious` (deferred) | `components == 1` with **`water_deep` (16)** treated as free — i.e. `free = !occupancy && !hard` over an ontology in which `water_deep` is neither. Requires the ontology change of D9 option 3, which moves `max(risk_raw)` 0.90 → 0.97. **Note this is *not* about `water_shallow`**: id 15 is already ρ 0.90, `hard = no`, `nav = rough` (§16.2), so shallow water is already in the free set under **every** policy including `forced-bridges`, and no policy is needed to wade. |
...
> **The N = 1 case, made explicit — the `wasm` preset could not otherwise pass its own gate.** `wasm` has one island, and the headline table (§0) puts it on *"forced-bridges (trivial at 1 island)"* with *"Forced isthmuses (N − 1) = 0"*. That has to actually pass, and as revision 2.1 wrote this row it could not: the width clause was unconditional, `world_narrowest_bridge_m` on a bridgeless world is the struct's `0.0` default, and `0.0 ≥ 16.0` is false on every seed. Pinned, at `N = 1`:
>
> ```
> world_bridges_expected   = 0                  # N - 1
> world_bridges_verified   = 0                  # and must EQUAL expected
> world_narrowest_bridge_m = nullopt            # UNDEFINED / NA -- null in the manifest,
>                                               # "n/a" in the UI. NOT 0.0, and NOT compared.
> world_components         == 1                 # STILL BINDING
> components (window)      == 1                 # STILL BINDING
> ```
>
> The width clause is skipped, not satisfied — the distinction matters, because a `0.0` sentinel silently compared against a floor is exactly how this bug arrived. The two component criteria stay binding at `N = 1`: a lone island can still be severed by a canyon, a cliff ring, an erosion channel or an authored paint stroke, which is what the gate is for. The same vacuity applies for any `N` under the `single-island` ablation (`force_bridges = false`), where `world_bridges_expected` is 0 by definition. A `null` bridge width in a manifest therefore means "this world has no isthmuses", never "nobody measured".
...
Surface ───────────────────────────────────────────────────────
 ...
 ─ Contract preview (the exact bytes that will be written) ─────
   ...
   outdoor   components 1  world 1  largest 100 %   policy forced-bridges
             WORLD bridges 2/2 verified  narrowest 16.5 m (>= 16.0 required)
             this window: 1 bridge, narrowest 17.4 m   [diagnostic, not gated]
             (1-island world: bridges 0/0, narrowest n/a -- clause vacuous)
...
| **whole-world** connectivity verify, 2 m lattice, full extent | 40 ms `standard` (2 048² = 4.2 M cells) / 150 ms `large` (4 096² = 16.8 M) | 12 ms (wasm, 1 024² = 1.05 M) | 3, once per world |
...
The world verify's transient buffers are the only new allocation of any size: 1 B of `uint8` occupancy plus a 32-bit EDT scratch = **5 B/cell**, over the 2 m lattice's cell count — 16.8 M × 5 B ≈ **84 MB** (`large`), 4.2 M × 5 B ≈ **21 MB** (`standard`), 1.05 M × 5 B ≈ **5 MB** (wasm) — allocated inside `validate_world` and **freed before it returns**. (The per-preset cell counts are §7.8's, corrected: `standard` is 2 048 cells across at 2 m, not 4 096. These three memory figures and the three timings above were already computed from the correct counts and are unchanged; only §7.8's prose was wrong.) It is deliberately absent from §11.3, which budgets *resident* memory for the running renderer; a generation-time peak that does not survive the call is a different quantity and conflating the two is how memory tables start lying.
...
- **Whole-world connectivity (D9)** — for all three presets × 64 seeds, `validate_world` reports `world_components == 1` and `world_bridges_verified == world_bridges_expected == islands − 1`; and, **where `islands > 1`**, `world_narrowest_bridge_m ≥ 2·inflate_m + 4.0`. This is the test that makes the user's decision an enforced property rather than a paragraph.
- **The N = 1 world passes its own gate** — asserted explicitly on `wasm` (1 island) across the same 64 seeds: `world_bridges_expected == world_bridges_verified == 0`, `world_narrowest_bridge_m` is `nullopt` (`null` in the manifest, never `0.0`), the width clause is **not evaluated**, and `world_components == 1` and window `components == 1` still hold. Written because revision 2.1's acceptance row compared a bridgeless world's `0.0` against a 16.0 m floor, which no single-island preset could ever pass.
...
- **Coarse lattice never invents a crossing** — the property `validate_world` actually rests on, in the only form that is sound. Over 32 random worlds, label the *inflated*-free set at 2 m and at 0.5 m over the same extent and assert that **no two cells the 2 m labelling places in one component lie in different components of the 0.5 m labelling**. This is the no-phantom-crossing direction of §7.8, and it is the one whose failure would ship an unsolvable corpus. Its converse is deliberately **not** asserted: an erosion can delete a whole fine component as well as sever one, so "the 2 m labelling never reports fewer components than the 0.5 m labelling" — the form this test had in revision 2.1 — is false in general and would fail on the first world carrying a sliver islet. The count comparison is kept only in its provable restriction: counting fine components that contain **at least one fully-free coarse cell**, `coarse_components ≥ that count`, which follows directly from the lifting property.
...
- **Ablation still works** — with `force_bridges = false`, the same worlds produce `world_bridges_expected == world_bridges_verified == 0` with `world_narrowest_bridge_m` `null` (the same vacuous-clause path as `N = 1`), and `single-island` windows that pass on `largest_component_fraction ≥ 0.98`. The ablation knob is covered, so it cannot rot while unused.
...
1. **A bridge does not survive to the raster.** A cliff eats one flank, an authored paint stroke re-drowns a crest, or an erosion pass cuts a channel through a causeway. Then `world_bridges_verified < world_bridges_expected` and the world is rejected.
...
| **3** | **Amphibious ontology** (`amphibious`) | **`water_deep` (16) stops being `hard`.** It leaves `hard` *and* `occupancy` (the two travel together, §7.3) and becomes the ontology's highest soft class — `nav = rough`, ρ **0.97** — so open water enters the free set and crossing it is expensive but legal. | **Not free.** It breaks the two invariants that make the registry auditable. `registry[k].hard ⇒ registry[k].rho == 0.0` (§7.2a) no longer *removes* water from `risk_raw`, so **`max(risk_raw)` rises 0.90 → 0.97** and §12.3's assertion, §16.2's note and §9.4's contract preview all move with it. | **It changes exported `hard` and `occupancy` bytes for every consumer**, and it asserts **swimming**, which is a domain claim about the agent, not a rendering choice. Corpora built under different policies are not comparable. |

> **A correction revision 2.1 owes the reader.** Revision 2 described option 3 as *"`water_shallow` stops being `hard`"*. **`water_shallow` (15) has never been `hard`** — §16.2 ships it at ρ 0.90, `hard = no`, `nav = rough`, and `puddle` (14) at ρ 0.85, `hard = no`. Wading through shallow water is therefore **already legal in the shipped ontology and requires no policy at all**; on the old wording, deferring option 3 deferred nothing. The only water class that blocks a ground agent is **`water_deep` (16)**, which is also where §7.4's layer-0 table and the water roadmap's §9.1 send every unfordable reach (`φ > d_ford`, or `φ·v ≥ 0.5 m²/s`). Option 3 is restated above against that class, which is the version that is genuinely deferred and genuinely changes exported bytes. It remains deferred, now for a reason that survives inspection.
...
world_lattice_m     = 2.0               # coarse lattice for the whole-world verify.
                                        # AND-coarsening of the INFLATED-free set,
                                        # in that order (7.8).
world_verify        = true              # run validate_world once per world. It is the
                                        # AUTHORITATIVE producer of world_bridges_* and
                                        # world_narrowest_bridge_m; validate_outdoor's
                                        # bridges_in_window / narrowest_bridge_in_window_m
                                        # are diagnostics and gate nothing.
                                        # At islands == 1 the bridge-width clause is
                                        # VACUOUS (expected == verified == 0, width null).
repair_attempts     = 3
resample_attempts   = 8
```
...
| **4** | **The whole-world verdict exists.** `validate_world` labels the full extent on a **2.0 m connectivity lattice** (an AND-coarsening of the inflated-free set, conservative in the crossing direction: coarse-connected ⇒ fine-connected, though *not* in component count), reports `world_components`, `world_bridges_expected` / `world_bridges_verified` and a fine-lattice `world_narrowest_bridge_m` (`null` at `N = 1`), and runs **once per world**, not per window. Window-vs-world is explained as a crop, and window repair builds its isthmus into the **shared delta grid** so the two verdicts can never drift. | §7.8 (`outdoor_gate_report` gains scope-tagged world/window fields; new `validate_world`) |
...
| **8** | **Six new tests**, so the decision is enforced rather than described: whole-world `components == 1` over 3 presets × 64 seeds; the `N = 1` (`wasm`) vacuous-clause case; an **inflation-does-not-sever-the-bridge** regression parameterised over `inflate_m ∈ {4, 6, 8}`; coarse-lattice no-phantom-crossing; `components == 1 ⟺ largest_component_fraction == 1.0`; and an ablation test so `force_bridges = false` cannot rot while unused. | §12.3; L2's row in §13.2 |
...
**Amendment, 2026-08-28 — D9 option 3 restated against the right class.** Revision 2.1 carried revision 2's description of `amphibious` unexamined: *"`water_shallow` stops being `hard` and becomes high-soft (ρ 0.90, `hard = false`)."* Checked against §16.2, **`water_shallow` (15) already ships at exactly ρ 0.90, `hard = no`, `nav = rough`** — as does `puddle` (14) at ρ 0.85 — so the option described a state the ontology is already in, and "option 3 stays deferred" deferred nothing. The class that actually blocks a ground agent is **`water_deep` (16)**, which is where §7.4's layer-0 table sends deep water and where the water roadmap's §9.1 sends every unfordable reach. Option 3 and the §7.8 `amphibious` row are restated against `water_deep`, including the consequence revision 2 never priced: it breaks the `hard ⇒ ρ = 0` invariant's coverage of water and moves `max(risk_raw)` **0.90 → 0.97**, which §12.3, §16.2's notes and §9.4's contract preview all assert. **It remains deferred** — now for a reason that survives inspection. Sibling propagation of the same defect is recorded in `WATER-RENDERING-ROADMAP.md` §15, revision 1.1, which also brings that document's connectivity policy from the superseded `single-island` wording to `forced-bridges` / `components == 1`. Changed here: §7.8 acceptance table (`amphibious` row), §15.3 D9 option 3 + a correction note.

**Amendment, 2026-08-28 — four defects inside the new §7.8 text, found by a verification pass.** The D9 propagation was sound; these are errors in the prose that carried it.

1. **Arithmetic.** §7.8 read *"At 2.0 m it is 4 096² = 16.8 M cells (4 096 cells across for `standard`, 1 024 for `wasm`)"*. 4 096 across is the **`large`** figure. `standard`'s extent is **4 096 m** (§4.2, §4.3), so at a 2.0 m lattice it is **2 048** across = **4.2 M** cells, not 16.8 M — a 4× overstatement. Corrected, with all three presets spelled out (`large` 4 096² = 16.8 M, `standard` 2 048² = 4.2 M, wasm 1 024² = 1.05 M). Chased into §11.2, where the timings (150 / 40 / 12 ms) and the transient-buffer figures (84 / 21 / 5 MB at 5 B/cell) turn out to have been derived from the **correct** counts all along and therefore do not move; the cell counts are now shown inline in the row and in the memory paragraph so the arithmetic is checkable where it is used. §11.3 is untouched by design — it budgets *resident* renderer memory and the world-verify scratch is transient.
2. **Inverted soundness argument.** The text claimed the AND-coarsening makes a coarse `components == 1` *imply* a fine `components == 1`, and §12.3 defended that with *"the 2 m labelling never reports fewer components than the 0.5 m labelling"* — which is **false**, because an erosion can *delete* a fine component (a sliver islet) as well as sever one, so the coarse count can legitimately be lower. Component count is not monotone under coarsening; the **crossing** property is. Rewritten to claim only what the subset relation gives — *every coarse crossing is a real fine crossing, so the coarse pass can never report CONNECTED where the fine lattice is severed*, which is exactly the conservative direction for a connectivity claim — with the residue bounded (anything a 2 m erosion erases is a free feature under `2 + 2·inflate_m = 14 m` wide, beneath the 16.0 m floor the gate would accept as a crossing at all) and the 0.5 m window gate named as the backstop. §12.3's test is restated to assert no-phantom-crossing, plus the count comparison only in its provable restriction. Inflation-then-coarsening order is now pinned, since the argument depends on it.
3. **The `wasm` preset could not pass its own gate.** §0 puts wasm on *"forced-bridges (trivial at 1 island)"* with 0 forced isthmuses, but the §7.8 acceptance row demanded `narrowest_bridge_m ≥ 16.0 m` unconditionally — and a bridgeless world's width is the struct's `0.0` default, so wasm failed on every seed. The `N = 1` case is now explicit: `world_bridges_expected = world_bridges_verified = 0`, `world_narrowest_bridge_m` **undefined** (`nullopt` / `null` / "n/a", never `0.0`), the width clause **vacuous rather than satisfied**, and `world_components == 1` and window `components == 1` **still binding** — a lone island can still be severed by a canyon or a paint stroke. The same vacuity covers the `single-island` ablation at any `N`. A new §12.3 test asserts it.
4. **Two producers, no authority.** `narrowest_bridge_m` and `bridges_verified` were produced both by `validate_world` (whole world, 2 m labelling, widths re-measured at 0.5 m) and by `validate_outdoor` (one window, 0.5 m), and neither the manifest nor the acceptance table said which. **`validate_world` is now authoritative** — it is the only producer that knows `N` and so the only one that can compute `bridges_expected = N − 1`. `validate_outdoor` reports the window's view as a **diagnostic that gates nothing**, because a legitimate 256 m window usually contains no bridge. Fields are renamed to carry their scope: `world_bridges_expected` / `world_bridges_verified` / `world_narrowest_bridge_m` versus `bridges_in_window` / `narrowest_bridge_in_window_m`, in the struct, the manifest example, the acceptance table, §16.3, §12.3, §13.2's risk defences and the §9.4 readouts.

Changed here: §4.3a.4 step 4; §7.8 (struct, both function contracts, the lattice paragraph, the new coarsening/soundness pair, the acceptance row, the new `N = 1` block, algorithm step 5); §7.5 manifest example + its field notes; §9.4 World tab readout and contract preview; §11.2 world-verify row and memory paragraph; §12.3 (two bullets rewritten, one added); §13.2 R19 defences; §16.3 outdoor-gate block; §17 rows 4 and 8.
