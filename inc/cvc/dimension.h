...
public:
  ...
  /*
    ...
    operator[]).
  */
  template <class C> explicit dimension(const C &m) : xdim(m[0]), ydim(m[1]), zdim(m[2]) {}

  // initialize from string
  explicit dimension(const std::string &s) { str(s); }
  ...
  bool operator<(const dimension &d) const { return (*this <= d) && (*this != d); }
  ...
  std::string str() const {
    ...
    return boost::str(boost::format("%1%,%2%,%3%") % xdim % ydim % zdim);
  }
  ...
  void str(const std::string &s) {
    ...
    if (parts.size() != 3)
      throw invalid_dimension_string(s);
    try {
      for (int i = 0; i < 3; i++) {
        trim(parts[i]);
