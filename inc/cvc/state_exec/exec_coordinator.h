...
public:
    ...
    submit_result submit(const std::string& script,
                         const execute_options& opts = {});
    submit_result submit(const char* script,
                         const execute_options& opts = {}) {
        return submit(std::string(script), opts);
    }
    submit_result submit(const value_t& expr,
                         const execute_options& opts = {});
    ...
    bool admin_set_policy(const resource_policy& policy,
    ...
    static bool deserialize_scheduler_stats(const std::string& json,
                                            scheduler_stats& out);
    static std::string serialize_process_info(const process_info& pi);
    static bool deserialize_process_info(const std::string& json,
                                         process_info& out);
    static std::string serialize_process_list(
        const std::vector<process_info>& procs);
    static bool deserialize_process_list(
        const std::string& json, std::vector<process_info>& out);
    static std::string serialize_resource_policy(const resource_policy& p);
    static bool deserialize_resource_policy(const std::string& json,
                                            resource_policy& out);
...
private:
    ...
    // Submit helpers
    submit_result submit_local(const std::string& script,
                               const execute_options& opts);
    submit_result submit_local(const value_t& expr,
                               const execute_options& opts);
    void send_submit_response(const std::string& reply_path,
    ...
    // Message sending
    void send_message(const std::string& path,
    ...
    std::unordered_map<std::string,
        std::chrono::steady_clock::time_point> remote_last_seen_;
