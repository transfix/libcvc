...
public:
  volume(app &ctx, const dimension &d = dimension(4, 4, 4), data_type vt = UChar,
         const bounding_box &box = bounding_box(-0.5, -0.5, -0.5, 0.5, 0.5, 0.5))
      : voxels(ctx, d, vt), _boundingBox(box), _desc("No Name") {}
  volume(app &ctx, const unsigned char *v, const dimension &d, data_type vt,
         const bounding_box &box = bounding_box(-0.5, -0.5, -0.5, 0.5, 0.5, 0.5))
      : voxels(ctx, v, d, vt), _boundingBox(box), _desc("No Name") {}
  volume(const voxels &vox, const bounding_box &box = bounding_box(-0.5, -0.5, -0.5, 0.5, 0.5, 0.5))
      : voxels(vox), _boundingBox(box), _desc("No Name") {}
  volume(const volume &vol) : voxels(vol), _boundingBox(vol.boundingBox()), _desc(vol.desc()) {}
  volume(app &ctx, const std::string &filename) : voxels(ctx) { read(filename); }
  ~volume() {}
  ...
  double XSpan() const {
    return XDim() - 1 == 0 ? 0.0 : (boundingBox().maxx - boundingBox().minx) / (XDim() - 1);
  }
  double YSpan() const {
    return YDim() - 1 == 0 ? 0.0 : (boundingBox().maxy - boundingBox().miny) / (YDim() - 1);
  }
  double ZSpan() const {
    return ZDim() - 1 == 0 ? 0.0 : (boundingBox().maxz - boundingBox().minz) / (ZDim() - 1);
  }
  ...
  */
  ...
  bool operator==(const volume &vol) {
    return voxels::operator==(vol) && boundingBox() == vol.boundingBox();
  }

  bool operator!=(const volume &vol) { return !(*this == vol); }

  /*
    Operations!
  */
  virtual volume &copy(const volume &vol);
  virtual volume &sub(uint64 off_x, uint64 off_y, uint64 off_z, const dimension &subvoldim
#ifdef _MSC_VER
                      ,
  ...
  /*
    compose volumes using object space coordinates.  Makes a duplicate of compVol and resizes it to
    match the grid resolution of this volume, then does normal voxel composition.
  */
  // virtual volume& compositeObj(const volume& compVol, double off_x, double off_y, double off_z,
  // const CompositeFunction& func);
  ...
  // returns a linearly interpolated voxel value for the object coordinates supplied.  The
  // coordinates must be inside the bounding box, or an exception is thrown.
  ...
  // resizes the volume to a new bounding box using trilinear interpolation (GPU-accelerated when
  // CUDA is available)
  volume &resize(const bounding_box &new_bbox);

  // makes this volume into a new volume that contains both this volume and the volume specified,
  // bounding box and all If dimension is specified, this volume will be resized to that dimension
  volume &combineWith(const volume &vol, const dimension &dim);
  volume &combineWith(const volume &vol);

  virtual volume &read(const std::string &filename, unsigned int var = 0, unsigned int time = 0,
