...
ssim_result vol_ssim(const volume &a, const volume &b, int window_size = 11, double sigma = 1.5);
...
volume vol_project(const volume &vol, const std::vector<double> &angles_rad, double step = 0.5);
...
volume vol_back_project(const volume &projections, const std::vector<double> &angles_rad,
                        unsigned int output_dim, bool apply_filter = true);
...
void vol_to_slices(const volume &vol, const std::string &directory,
