...
/**
 * @class voxels
 * @brief Multi-dimensional voxel data container with configurable copy
 * semantics
 *
 * IMPORTANT - MEMORY SEMANTICS:
 * =============================
 * Voxels uses SHALLOW COPY semantics by default via boost::shared_array for
 * CPU memory and std::shared_ptr for CUDA unified memory (with automatic
 * reference counting).
 *
 ...
 * - copy() method: v2.copy(v1);              // Shares CPU & GPU memory
 * (default)
 * - copy() method: v2.copy(v1, false);       // Shares CPU & GPU memory
 * (explicit)
 *
 ...
 *
 * CUDA MEMORY: When CUDA is enabled, both CPU and GPU memory are reference
 * counted. The custom CudaManagedDeleter ensures CUDA unified memory is
 ...
 * - sub() method: v2.sub(0, 0, 0, v1.voxel_dimensions());  // Creates
 * independent copy
 *
 * With deep copy, each copy has its own independent voxel data array.
 * Modifications to one copy will NOT affect other copies.
 *
 * This design enables efficient passing without copying large arrays by
 * default, while still providing explicit deep copy when needed.
 */
class voxels {
public:
  ...
  */
  ...
  bool operator==(const voxels &vox) const {
    ...
    if (voxel_dimensions() != vox.voxel_dimensions())
      return false;
    if (voxelType() != vox.voxelType())
      return false;

    // Check if same shared_array pointer (shared data)
    if (_voxels.get() == vox._voxels.get())
      return true;
...
protected:
  ...
  // Single shared_array for voxel data (all types stored as bytes, cast as
  // needed)
