...
namespace CVC_NAMESPACE
{
  ...
  */
  ...
  public:
    ...
    generic_bounding_box(const std::string& s)
      {
        str(s);
      }
	
    /*
      Universal explicit converter from any class to generic_bounding_box (as long as that class implements
      operator[]).
    */
...
#undef REALMIN
	...
	*/
    ...
    void normalize()
    {
      *this = generic_bounding_box<T>(std::min(minx,maxx),
				    std::min(miny,maxy),
				    std::min(minz,maxz),
				    std::max(minx,maxx),
				    std::max(miny,maxy),
    ...
    void str(const std::string& s)
    {
      ...
      if(parts.size()!=6) throw invalid_bounding_box_string(s);
      try
        {
          for(int i = 0; i < 6; i++)
            {
