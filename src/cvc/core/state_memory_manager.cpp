...
state_memory_manager::state_memory_manager(state &root, std::size_t budget_bytes,
                                           eviction_policy policy,
                                           std::unique_ptr<state_eviction_store> store)
    : _root(root), _budget(budget_bytes), _policy(policy),
      _store(store ? std::move(store) : std::make_unique<memory_eviction_store>()) {}
...
void state_memory_manager::track(const std::string &path, std::size_t payload_bytes) {
  std::lock_guard<std::mutex> lk(_mu);
...
std::size_t state_memory_manager::evict_one_locked() {
  ...
  case eviction_policy::lfu:
    ...
    victim = std::min_element(
        ...
        [](const node_entry &a, const node_entry &b) { return a.access_count < b.access_count; });
    break;
  ...
  case eviction_policy::ttl: {
    ...
    for (auto it = _lru_list.begin(); it != _lru_list.end(); ++it) {
      if (now - it->tracked_since >= _ttl) {
        if (victim == _lru_list.end() || it->tracked_since < victim->tracked_since)
          victim = it;
      }
    }
