...
struct null_io : public volume_file_io {
  ...
  // ---- Change History ----
  // 12/04/2009 -- Joe R. -- Creation.
  null_io() : _id("null_io : v1.0") { _extensions.push_back(".nothing"); }
  ...
  virtual void getVolumeFileInfo(app & /*ctx*/, volume_file_info::data & /*data*/,
    ...
    throw read_error(boost::str(boost::format("%1% unimplemented") % BOOST_CURRENT_FUNCTION));
  }
  ...
  virtual void createVolumeFile(app & /*ctx*/, const std::string & /*filename*/,
    ...
    throw write_error(boost::str(boost::format("%1% unimplemented") % BOOST_CURRENT_FUNCTION));
  }
