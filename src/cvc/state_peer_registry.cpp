...
bool state_peer_registry::any_prefix_matches(
    const std::vector<std::string> &prefixes,
    const std::string &path) noexcept {
  if (prefixes.empty()) return true;
  for (const auto &p : prefixes) {
    if (prefix_matches(p, path)) return true;
  }
  return false;
}

void state_peer_registry::add_peer(std::string node_id,
...
bool state_peer_registry::set_subscriptions(
    const std::string &node_id, std::vector<std::string> subscriptions) {
  std::lock_guard<std::mutex> lk(_mu);
...
bool state_peer_registry::should_deliver(const std::string &node_id,
  ...
  std::lock_guard<std::mutex> lk(_mu);
  auto it = _peers.find(node_id);
  if (it == _peers.end()) return true;  // unknown peer => back-compat default
  return any_prefix_matches(it->second.subscriptions, path);
}

bool state_peer_registry::note_seen(const std::string &node_id,
                                    std::uint64_t now_ns) {
  std::lock_guard<std::mutex> lk(_mu);
  auto it = _peers.find(node_id);
  if (it == _peers.end()) return false;
...
void state_peer_registry::note_mutation_delivered(
    const std::string &node_id) {
  std::lock_guard<std::mutex> lk(_mu);
  auto it = _peers.find(node_id);
  if (it != _peers.end()) ++it->second.mutations_delivered;
}

void state_peer_registry::note_message_delivered(
    const std::string &node_id) {
  std::lock_guard<std::mutex> lk(_mu);
  auto it = _peers.find(node_id);
  if (it != _peers.end()) ++it->second.messages_delivered;
}

void state_peer_registry::note_delivery_filtered(
    const std::string &node_id) {
  std::lock_guard<std::mutex> lk(_mu);
  auto it = _peers.find(node_id);
  if (it != _peers.end()) ++it->second.deliveries_filtered;
}
...
std::vector<state_peer_registry::peer>
state_peer_registry::snapshot() const {
  std::lock_guard<std::mutex> lk(_mu);
  std::vector<peer> out;
  out.reserve(_peers.size());
  for (const auto &kv : _peers) out.push_back(kv.second);
  return out;
}
