...
async_evaluator::async_evaluator(environment_ptr global_env)
...
task<value_t> async_evaluator::evaluate(
...
task<value_t> async_evaluator::evaluate_script(
    const std::string& script, environment_ptr env)
{
    auto exprs = parse_all(script);
    if (exprs.empty()) co_return nil_value;
    if (exprs.size() == 1)
...
value_t async_evaluator::sync_evaluate(
    const value_t& expr, environment_ptr env,
...
value_t async_evaluator::sync_evaluate_script(
    const std::string& script, environment_ptr env,
    std::optional<double> timeout_sec,
    std::function<void(const value_t&)> on_complete)
{
    auto exprs = parse_all(script);
    if (exprs.empty()) return nil_value;
    if (exprs.size() == 1)
    ...
    begin_exprs.push_back(value_t{symbol{"begin"}});
    begin_exprs.insert(begin_exprs.end(), exprs.begin(), exprs.end());
    return sync_evaluate(make_list(std::move(begin_exprs)),
                         env, timeout_sec, std::move(on_complete));
}
...
bool async_evaluator::is_paused() const {
...
task<value_t> async_evaluator::eval_internal(
    const value_t& expr, environment_ptr env)
{
    ...
    if (auto* sym = std::get_if<symbol>(&expr.v)) {
        if (sym->name == "nil") co_return nil_value;
        if (sym->name == "t")   co_return true_value;
        if (auto* val = env->lookup(sym->name)) co_return *val;
    ...
    if (!lp || !*lp) co_return expr;
    auto& elems = **lp;
    if (elems.empty()) co_return make_list();
    ...
    // --- Special forms ---
    if (name == "if")       co_return co_await do_if(args, env);
    if (name == "begin")    co_return co_await do_begin(args, env);
    if (name == "while")    co_return co_await do_while(args, env);
    if (name == "for")      co_return co_await do_for(args, env);
    if (name == "set")      co_return co_await do_set(args, env);
    if (name == "quote")    co_return co_await do_quote(args);
    if (name == "lambda")   co_return co_await do_lambda(args, env);
    if (name == "return")   co_return co_await do_return(args, env);
    if (name == "let")      co_return co_await do_let(args, env);
    if (name == "super")    co_return co_await do_super(args, env);
    if (name == "defun")    co_return co_await do_defun(args, env);
    if (name == "defclass") co_return co_await do_defclass(args, env);
    if (name == "defmacro") co_return co_await do_defmacro(args, env);
    if (name == "eval")     co_return co_await do_eval(args, env);
    if (name == "root")     co_return co_await do_root();
    if (name == "await")    co_return co_await do_await(args, env);
...
task<value_t> async_evaluator::do_if(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() < 2)
...
task<value_t> async_evaluator::do_begin(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.empty()) co_return nil_value;
...
task<value_t> async_evaluator::do_while(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() < 2)
...
task<value_t> async_evaluator::do_for(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() < 3)
    ...
    if (!lp) throw std::runtime_error("for: second arg must evaluate to list");
    for (auto& item : **lp) {
        co_await check_interrupted_async();
...
task<value_t> async_evaluator::do_set(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() != 2)
        throw std::runtime_error("set: expected 2 arguments");
    auto* sym = std::get_if<symbol>(&args[0].v);
    if (!sym) throw std::runtime_error("set: first arg must be symbol");
...
task<value_t> async_evaluator::do_lambda(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() < 2)
    ...
    } else if (auto* nil_sym = std::get_if<symbol>(&args[0].v);
               nil_sym && nil_sym->name == "nil") {
        // empty params
...
task<value_t> async_evaluator::do_return(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.empty())
...
task<value_t> async_evaluator::do_let(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() < 2)
    ...
    for (auto& b : **bindings) {
        ...
        if (!sym) throw std::runtime_error("let: binding name must be symbol");
...
task<value_t> async_evaluator::do_super(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() < 2)
    ...
    if (method_sym) method_name = method_sym->name;
    else method_name = std::get<std::string>((co_await eval_internal(args[1], env)).v);
...
task<value_t> async_evaluator::do_defun(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() < 3)
...
task<value_t> async_evaluator::do_defclass(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.empty())
...
task<value_t> async_evaluator::do_defmacro(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() < 3)
        throw std::runtime_error("defmacro: expected at least 3 arguments");
    auto* name_sym = std::get_if<symbol>(&args[0].v);
    if (!name_sym) throw std::runtime_error("defmacro: name must be symbol");
...
task<value_t> async_evaluator::do_eval(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() != 1)
...
task<value_t> async_evaluator::do_root() {
    co_return root_expr_;
}

task<value_t> async_evaluator::do_await(
    const std::vector<value_t>& args, environment_ptr env) {
    if (args.size() != 1)
...
task<value_t> async_evaluator::apply_closure(
    const closure_ptr& cls, const std::vector<value_t>& args)
{
    ...
    if (cls->variadic) {
        ...
        local->set(cls->params.back().name, make_list(
            std::vector<value_t>(args.begin(), args.end())));
    } else {
...
value_t async_evaluator::substitute(
    const value_t& tmpl,
