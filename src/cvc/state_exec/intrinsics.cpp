...
void require_root(const intrinsics_context* ctx, const char* name) {
    if (!ctx->root)
        throw std::runtime_error(
            std::string(name) + ": no state root bound");
}

void require_sched(const intrinsics_context* ctx, const char* name) {
    if (!ctx->sched)
        throw std::runtime_error(
            std::string(name) + ": no scheduler bound");
}
...
value_t intrinsic_state_get(intrinsics_context* ctx,
                            std::span<const value_t> args) {
    ...
    if (!node) return nil_value;
    return value_t(node->value());
}

value_t intrinsic_state_set(intrinsics_context* ctx,
                            std::span<const value_t> args) {
...
value_t intrinsic_state_delete(intrinsics_context* ctx,
                               std::span<const value_t> args) {
...
value_t intrinsic_state_data_get(intrinsics_context* ctx,
                                 std::span<const value_t> args) {
...
value_t intrinsic_state_data_set(intrinsics_context* ctx,
                                 std::span<const value_t> args) {
...
value_t intrinsic_spawn(intrinsics_context* ctx,
                        std::span<const value_t> args) {
    expect_min(args, 1, "spawn");
    require_sched(ctx, "spawn");
    auto& script = as_string(args[0], "spawn");
    execute_options opts;
    if (args.size() > 1) opts.name = as_string(args[1], "spawn");
    if (args.size() > 2) opts.priority = static_cast<int>(as_int(args[2], "spawn"));
    opts.uid = ctx->uid;
    int pid = ctx->sched->execute(script, opts);
    return value_t(static_cast<int64_t>(pid));
}

value_t intrinsic_fork(intrinsics_context* ctx,
                       std::span<const value_t> args) {
...
value_t intrinsic_self_pid(intrinsics_context* ctx,
                           std::span<const value_t> args) {
    expect_exact(args, 0, "self-pid");
    return value_t(static_cast<int64_t>(ctx->pid));
}

value_t intrinsic_self_uid(intrinsics_context* ctx,
                           std::span<const value_t> args) {
    expect_exact(args, 0, "self-uid");
    return value_t(ctx->uid);
}

value_t intrinsic_kill(intrinsics_context* ctx,
                       std::span<const value_t> args) {
...
value_t intrinsic_pause(intrinsics_context* ctx,
                        std::span<const value_t> args) {
...
value_t intrinsic_resume(intrinsics_context* ctx,
                         std::span<const value_t> args) {
...
value_t intrinsic_ps(intrinsics_context* ctx,
                     std::span<const value_t> args) {
    ...
    for (auto& pi : procs) {
        ...
        entries.emplace_back("status", value_t(std::string(
            pi.status == process_status::ready       ? "ready" :
            ...
            pi.status == process_status::killed        ? "killed" : "unknown")));
...
value_t intrinsic_inspect(intrinsics_context* ctx,
                          std::span<const value_t> args) {
    ...
    entries.emplace_back("status",       value_t(std::string(
        ...
        info->status == process_status::killed        ? "killed" : "unknown")));
...
value_t intrinsic_memory_usage(intrinsics_context* ctx,
                               std::span<const value_t> args) {
    expect_exact(args, 0, "memory-usage");
    if (!ctx->tracker) return value_t(int64_t(0));
    return value_t(static_cast<int64_t>(ctx->tracker->current_bytes(ctx->pid)));
}

value_t intrinsic_memory_limit(intrinsics_context* ctx,
                               std::span<const value_t> args) {
    expect_exact(args, 0, "memory-limit");
    if (!ctx->proc) return value_t(int64_t(0));
    return value_t(static_cast<int64_t>(ctx->proc->max_memory));
}

value_t intrinsic_time_elapsed(intrinsics_context* ctx,
                               std::span<const value_t> args) {
    expect_exact(args, 0, "time-elapsed");
    if (!ctx->proc) return value_t(0.0);
    return value_t(ctx->proc->elapsed_time());
}

value_t intrinsic_time_limit(intrinsics_context* ctx,
                             std::span<const value_t> args) {
...
value_t intrinsic_message_count(intrinsics_context* ctx,
                                std::span<const value_t> args) {
...
value_t intrinsic_message_limit(intrinsics_context* ctx,
                                std::span<const value_t> args) {
...
value_t intrinsic_step_count(intrinsics_context* ctx,
                             std::span<const value_t> args) {
...
value_t intrinsic_cluster_id(intrinsics_context* ctx,
                             std::span<const value_t> args) {
    expect_exact(args, 0, "cluster-id");
    return value_t(ctx->cluster_id);
}

value_t intrinsic_node_id(intrinsics_context* ctx,
                          std::span<const value_t> args) {
    expect_exact(args, 0, "node-id");
    return value_t(ctx->node_id);
}

value_t intrinsic_scheduler_stats(intrinsics_context* ctx,
                                  std::span<const value_t> args) {
...
value_t intrinsic_state_expire(intrinsics_context* ctx,
                               std::span<const value_t> args) {
    ...
    node->expireAfter(boost::posix_time::milliseconds(
        static_cast<int64_t>(secs * 1000.0)));
    return nil_value;
}

value_t intrinsic_state_expire_at(intrinsics_context* ctx,
                                  std::span<const value_t> args) {
...
value_t intrinsic_state_has_expiry(intrinsics_context* ctx,
                                   std::span<const value_t> args) {
...
value_t intrinsic_state_is_expired(intrinsics_context* ctx,
                                   std::span<const value_t> args) {
...
value_t intrinsic_state_clear_expiry(intrinsics_context* ctx,
                                     std::span<const value_t> args) {
...
value_t intrinsic_state_sweep(intrinsics_context* ctx,
                              std::span<const value_t> args) {
    if (args.size() > 1)
        throw std::runtime_error(
            "state-sweep-expired: expected 0-1 argument(s), got " +
            std::to_string(args.size()));
    ...
    if (args.size() == 1) {
        ...
        if (!target) return value_t(int64_t(0));
    }
...
value_t intrinsic_msg_send(intrinsics_context* ctx,
                           std::span<const value_t> args) {
    ...
    std::vector<std::pair<std::string, value_t>> entries;
    entries.emplace_back("status", value_t(std::string(
        result.status == cvc::state::send_message_result::status_kind::delivered
            ? "delivered" : "error")));
