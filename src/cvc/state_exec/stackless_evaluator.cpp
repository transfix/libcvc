...
evaluator_state stackless_evaluator::create_state(
    const value_t& expr, environment_ptr env)
{
...
evaluator_state stackless_evaluator::create_state(
    const std::string& script, environment_ptr env)
{
...
bool stackless_evaluator::step(evaluator_state& state) {
    ...
    switch (frame.phase) {
        case eval_phase::init:          step_init(state, frame);          break;
        case eval_phase::eval_args:     step_eval_args(state, frame);     break;
        case eval_phase::apply:         step_apply(state, frame);         break;
        case eval_phase::if_cond:       step_if_cond(state, frame);       break;
        case eval_phase::if_branch:     step_if_branch(state, frame);     break;
        case eval_phase::begin_next:    step_begin_next(state, frame);    break;
        case eval_phase::while_cond:    step_while_cond(state, frame);    break;
        case eval_phase::while_body:    step_while_body(state, frame);    break;
        case eval_phase::for_body:      step_for_body(state, frame);      break;
        case eval_phase::set_value:     step_set_value(state, frame);     break;
        case eval_phase::let_bindings:  step_let_bindings(state, frame);  break;
        case eval_phase::let_body:      step_let_body(state, frame);      break;
        case eval_phase::return_value:  step_return_value(state, frame);  break;
        case eval_phase::eval_inner:    step_eval_inner(state, frame);    break;
        case eval_phase::super_args:    step_super_args(state, frame);    break;
        case eval_phase::defclass_methods: /* handled inline */ break;
    }
    return state.done;
}

value_t stackless_evaluator::run(
...
value_t stackless_evaluator::evaluate(
    const value_t& expr, environment_ptr env,
    std::optional<double> timeout_sec,
    std::function<void(const value_t&)> on_complete)
{
...
value_t stackless_evaluator::evaluate_script(
    const std::string& script, environment_ptr env,
    std::optional<double> timeout_sec,
    std::function<void(const value_t&)> on_complete)
{
...
bool stackless_evaluator::is_paused() const {
...
void stackless_evaluator::push_frame(evaluator_state& state, value_t expr,
                                     environment_ptr env) {
...
void stackless_evaluator::step_init(evaluator_state& state, eval_frame& frame) {
    ...
    if (auto* sym = std::get_if<symbol>(&expr.v)) {
        if (sym->name == "nil") { pop_frame(state, nil_value); return; }
        if (sym->name == "t")   { pop_frame(state, true_value); return; }
        auto* val = env->lookup(sym->name);
        if (!val) throw std::runtime_error("undefined symbol: " + sym->name);
        pop_frame(state, *val);
        return;
    }
    ...
    if (!lp || !*lp) { pop_frame(state, expr); return; }
    auto& elems = **lp;
    if (elems.empty()) { pop_frame(state, make_list()); return; }
    ...
    // --- Special forms ---
    if (name == "if") {
        if (args.size() < 2) throw std::runtime_error("if: expected at least 2 args");
    ...
    if (name == "begin") {
        if (args.empty()) { pop_frame(state, nil_value); return; }
    ...
    if (name == "while") {
        if (args.size() < 2) throw std::runtime_error("while: expected 2 args");
    ...
    if (name == "for") {
        if (args.size() < 3) throw std::runtime_error("for: expected 3 args");
        auto* var_sym = std::get_if<symbol>(&args[0].v);
        if (!var_sym) throw std::runtime_error("for: first arg must be symbol");
    ...
    if (name == "set") {
        if (args.size() != 2) throw std::runtime_error("set: expected 2 args");
        auto* var_sym = std::get_if<symbol>(&args[0].v);
        if (!var_sym) throw std::runtime_error("set: first arg must be symbol");
    ...
    if (name == "quote") {
        if (args.size() != 1) throw std::runtime_error("quote: expected 1 arg");
        pop_frame(state, args[0]);
        return;
    }
    if (name == "lambda") {
        if (args.size() < 2) throw std::runtime_error("lambda: expected at least 2 args");
        std::vector<symbol> params;
        bool variadic = false;
        if (auto* pl = std::get_if<list_ptr>(&args[0].v); pl && *pl) {
            for (auto& p : **pl) {
                auto* ps = std::get_if<symbol>(&p.v);
                if (!ps) throw std::runtime_error("lambda: param must be symbol");
                if (ps->name == "&rest") { variadic = true; continue; }
                params.push_back(*ps);
            }
        }
    ...
    if (name == "return") {
        if (args.empty()) throw std::runtime_error("return: expected 1 arg");
        frame.phase = eval_phase::return_value;
        push_frame(state, args[0], env);
        return;
    }
    if (name == "let") {
        if (args.size() < 2) throw std::runtime_error("let: expected at least 2 args");
        auto* bindings_lp = std::get_if<list_ptr>(&args[0].v);
        if (!bindings_lp) throw std::runtime_error("let: first arg must be binding list");
    ...
    if (name == "defun") {
        if (args.size() < 3) throw std::runtime_error("defun: expected at least 3 args");
        auto* name_sym = std::get_if<symbol>(&args[0].v);
        if (!name_sym) throw std::runtime_error("defun: first arg must be symbol");
        ...
        if (auto* pl = std::get_if<list_ptr>(&args[1].v); pl && *pl) {
            for (auto& p : **pl) {
                auto* ps = std::get_if<symbol>(&p.v);
                if (!ps) throw std::runtime_error("defun: param must be symbol");
                if (ps->name == "&rest") { variadic = true; continue; }
                params.push_back(*ps);
            }
        }
    ...
    if (name == "defmacro") {
        if (args.size() < 3) throw std::runtime_error("defmacro: expected at least 3 args");
        auto* name_sym = std::get_if<symbol>(&args[0].v);
        if (!name_sym) throw std::runtime_error("defmacro: first arg must be symbol");
        std::vector<symbol> params;
        if (auto* pl = std::get_if<list_ptr>(&args[1].v); pl && *pl) {
            for (auto& p : **pl) {
                auto* ps = std::get_if<symbol>(&p.v);
                if (!ps) throw std::runtime_error("defmacro: param must be symbol");
                params.push_back(*ps);
            }
        }
        value_t body = (args.size() == 3)
            ? args[2]
            : make_list([&]{
    ...
    if (name == "eval") {
        if (args.size() != 1) throw std::runtime_error("eval: expected 1 arg");
    ...
    if (name == "super") {
        if (args.size() < 2) throw std::runtime_error("super: expected at least 2 args");
...
void stackless_evaluator::step_apply(evaluator_state& state, eval_frame& frame) {
    ...
    if (!head_sym) throw std::runtime_error("apply: no function head");
...
void stackless_evaluator::step_for_body(evaluator_state& state, eval_frame& frame) {
    if (frame.index == -1) {
        ...
        if (!lp) throw std::runtime_error("for: collection must be a list");
...
void stackless_evaluator::step_super_args(evaluator_state& state, eval_frame& frame) {
    ...
    if (static_cast<size_t>(frame.index) < remaining.size()) {
        ...
        for (auto& [k, v] : obj_dict)
            if (k == "__class__") { cls_val = &v; break; }
        if (!cls_val) throw std::runtime_error("super: no __class__");
        auto& cls_dict = *std::get<dict_ptr>(cls_val->v);
        value_t* super_val = nullptr;
        for (auto& [k, v] : cls_dict)
            if (k == "__super__") { super_val = &v; break; }
        if (!super_val) throw std::runtime_error("super: no __super__");
...
void stackless_evaluator::call_closure(
    evaluator_state& state, eval_frame& /*frame*/,
    const closure_ptr& cls, const std::vector<value_t>& args)
{
    ...
    if (cls->variadic) {
        ...
        local->set(cls->params.back().name,
                   make_list(std::vector<value_t>(args.begin(), args.end())));
    } else {
        if (args.size() != cls->params.size())
            throw std::runtime_error(
                "closure: expected " + std::to_string(cls->params.size()) +
...
void stackless_evaluator::handle_defclass(
    evaluator_state& state, eval_frame& frame,
    const std::vector<value_t>& args)
{
    ...
    auto* name_sym = std::get_if<symbol>(&args[0].v);
    if (!name_sym) throw std::runtime_error("defclass: first arg must be symbol");
    ...
    if (args.size() > 1) {
        if (auto* parent_sym = std::get_if<symbol>(&args[1].v)) {
            ...
            if (parent_val) {
                ...
                if (cd) super_cls_dict = std::get<dict_ptr>(cd->v);
            }
            methods_start = 2;
    ...
    auto has_method = [&](const std::string& n) {
        for (auto& m : method_names) if (m == n) return true;
        return false;
    };
    ...
    for (size_t i = methods_start; i < args.size(); ++i) {
        ...
        if (auto* pl = std::get_if<list_ptr>(&mlist[1].v); pl && *pl) {
            for (auto& p : **pl) {
                auto* ps = std::get_if<symbol>(&p.v);
                if (!ps) throw std::runtime_error("defclass: param must be symbol");
                if (ps->name == "&rest") { variadic = true; continue; }
                params.push_back(*ps);
            }
        }
        ...
        native_fn wrapper = [self, mcls, genv]
                            (std::span<const value_t> call_args) -> value_t {
    ...
    native_fn constructor = [cls_dict, cls_dict_val]
                            (std::span<const value_t> init_args) -> value_t {
...
value_t stackless_evaluator::substitute(
    const value_t& tmpl,
