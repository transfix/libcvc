...
bool xint(const std::string& json, const char* key, int& out) {
    int64_t v = 0;
    if (!xi64(json, key, v)) return false;
...
std::string make_path(const std::string& cluster_id,
...
void exec_coordinator::attach_scheduler(scheduler* sched) {
    sched_ = sched;
}

void exec_coordinator::attach_shard(cvc::state_cluster_shard* shard) {
    shard_ = shard;
}

void exec_coordinator::attach_membership(cvc::state_cluster_membership* m) {
    membership_ = m;
}

void exec_coordinator::attach_message_bus(cvc::state_message_bus* bus) {
    bus_ = bus;
}
...
void exec_coordinator::set_config(const config& cfg) {
    cfg_ = cfg;
}

void exec_coordinator::set_node_id(const std::string& id) {
    node_id_ = id;
}

void exec_coordinator::set_cluster_id(const std::string& id) {
    cluster_id_ = id;
}
...
void exec_coordinator::start() {
    ...
    bus_sub_id_ = bus_->subscribe(prefix,
        [this](const cvc::state_message& m) { on_message(m); });
...
void exec_coordinator::request_election() {
    start_election();
}
...
void exec_coordinator::start_election() {
    ...
    send_message(make_path(cluster_id_, "election"),
                 MIME_EXEC_ELECTION, os.str());
}
...
void exec_coordinator::handle_election(const cvc::state_message& msg) {
    ...
    if (type == "election-start") {
        if (sender == node_id_) return; // ignore own start messages
        ...
        bool we_win = (cfg_.election_priority > sender_pri) ||
                      (cfg_.election_priority == sender_pri &&
                       node_id_ > sender);
        ...
        if (sender == node_id_) return; // ignore own alive messages
        ...
        bool they_win = (sender_pri > cfg_.election_priority) ||
                        (sender_pri == cfg_.election_priority &&
                         sender > node_id_);
        if (they_win) {
            election_in_progress_ = false;
        }
...
void exec_coordinator::emit_heartbeat() {
    ...
    send_message(make_path(cluster_id_, "heartbeat"),
                 MIME_EXEC_HEARTBEAT, os.str());
...
void exec_coordinator::emit_status_broadcast() {
    ...
    send_message(make_path(cluster_id_, "status"),
                 MIME_EXEC_STATUS, os.str());
...
submit_result exec_coordinator::submit(const std::string& script,
                                       const execute_options& opts) {
    if (!sched_) return {-1, false, "no scheduler attached", ""};
    ...
    send_message(make_path(cluster_id_, "submit"),
                 MIME_EXEC_SUBMIT, os.str());

    // For forwarded submissions, we return a pending result
    return {-1, true, "", leader_node_id()};
}

submit_result exec_coordinator::submit(const value_t& expr,
                                       const execute_options& opts) {
    if (!sched_) return {-1, false, "no scheduler attached", ""};
...
submit_result exec_coordinator::submit_local(const std::string& script,
    ...
    process_limits req{opts.max_steps, opts.max_time,
                       opts.max_memory, opts.max_messages};
    ...
    if (policy_.max_processes > 0 &&
        sched_->process_count() >= policy_.max_processes) {
        std::lock_guard lk(stats_mu_);
...
submit_result exec_coordinator::submit_local(const value_t& expr,
                                             const execute_options& opts) {
    process_limits req{opts.max_steps, opts.max_time,
                       opts.max_memory, opts.max_messages};
    auto validated = validate_limits(policy_, req);
...
void exec_coordinator::handle_submit(const cvc::state_message& msg) {
    ...
    if (action == "submit") {
        ...
        if (!reply_node.empty()) {
            send_submit_response(
                make_path(cluster_id_, "submit.reply." + reply_node),
                result);
        }
...
migrate_result exec_coordinator::migrate(int pid,
                                         const std::string& target_node_id) {
    if (!sched_) return {false, -1, target_node_id, "no scheduler"};
    ...
    send_message(
        make_path(cluster_id_, "migrate." + target_node_id),
        MIME_EXEC_MIGRATE, os.str());
...
void exec_coordinator::handle_migrate(const cvc::state_message& msg) {
    ...
    if (action == "migrate-request") {
        ...
        send_message(
            make_path(cluster_id_, "migrate." + source_node),
            MIME_EXEC_MIGRATE, os.str());
...
bool exec_coordinator::admin_pause(int pid, const std::string& node_id) {
    ...
    send_message(make_path(cluster_id_, "control." + node_id),
                 MIME_EXEC_CONTROL, os.str());
    return true;
}
...
bool exec_coordinator::admin_set_policy(const resource_policy& policy,
    ...
    send_message(make_path(cid, "policy"),
                 MIME_EXEC_POLICY, os.str());
    return true;
}
...
void exec_coordinator::on_message(const cvc::state_message& msg) {
    ...
    const auto& ct = msg.content_type.empty()
        ? msg.effective_content_type()
        : msg.content_type;

    if (ct == MIME_EXEC_ELECTION)  { handle_election(msg);  return; }
    if (ct == MIME_EXEC_HEARTBEAT) { handle_heartbeat(msg); return; }
    if (ct == MIME_EXEC_SUBMIT)    { handle_submit(msg);    return; }
    if (ct == MIME_EXEC_MIGRATE)   { handle_migrate(msg);   return; }
    if (ct == MIME_EXEC_CONTROL)   { handle_control(msg);   return; }
    if (ct == MIME_EXEC_STATUS)    { handle_status(msg);    return; }
    if (ct == MIME_EXEC_POLICY)    { handle_policy(msg);    return; }
}
...
void exec_coordinator::send_message(const std::string& path,
...
std::string exec_coordinator::serialize_scheduler_stats(
    const scheduler_stats& s) {
    std::ostringstream os;
    os << '{';
    bool f = true;
...
bool exec_coordinator::deserialize_scheduler_stats(const std::string& json,
                                                   scheduler_stats& s) {
...
bool exec_coordinator::deserialize_process_info(const std::string& json,
                                                process_info& pi) {
    if (json.empty()) return false;
...
std::string exec_coordinator::serialize_process_list(
    const std::vector<process_info>& procs) {
    std::ostringstream os;
    os << '[';
...
bool exec_coordinator::deserialize_process_list(
    const std::string& json, std::vector<process_info>& out) {
    out.clear();
...
std::string exec_coordinator::serialize_resource_policy(
    const resource_policy& p) {
    std::ostringstream os;
    os << '{';
    bool f = true;
...
bool exec_coordinator::deserialize_resource_policy(const std::string& json,
                                                   resource_policy& p) {
    if (json.empty()) return false;
