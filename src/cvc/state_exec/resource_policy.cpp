...
template<typename T>
T clamp_limit(T val, T lo, T hi, const char* field,
              resource_policy::mode m) {
    ...
    if (effective_lo != T{0} && val != T{0} && val < effective_lo) {
        if (m == resource_policy::mode::strict)
            throw std::runtime_error(
                std::string("resource_policy: ") + field +
                " below minimum");
        val = effective_lo;
    }
...
template<typename T>
T apply_default(T val, T def) {
    return (val == T{0} && def != T{0}) ? def : val;
}

} // anonymous namespace

process_limits validate_limits(const resource_policy& policy,
    ...
    result.max_steps = clamp_limit(result.max_steps,
        policy.max_steps_min, policy.max_steps_max,
        "max_steps", policy.enforce);

    result.max_time = clamp_limit(result.max_time,
        policy.max_time_min, policy.max_time_max,
        "max_time", policy.enforce);

    result.max_memory = clamp_limit(result.max_memory,
        policy.max_memory_min, policy.max_memory_max,
        "max_memory", policy.enforce);

    result.max_messages = clamp_limit(result.max_messages,
        policy.max_messages_min, policy.max_messages_max,
        "max_messages", policy.enforce);
