...
evaluator::evaluator(environment_ptr global_env)
    ...
    std::function<void(const value_t&)> on_complete)
{
    ...
    if (!env) run_env = environment::extend(global_env_);
    ...
    try {
        ...
        if (timeout_sec) {
            ...
            worker.join();
            if (ex) { stats_.mark_complete(); std::rethrow_exception(ex); }
...
value_t evaluator::evaluate_script(
    ...
    std::function<void(const value_t&)> on_complete)
{
    ...
    return evaluate(make_list(std::move(begin_exprs)),
...
bool evaluator::is_paused() const {
...
value_t evaluator::eval_internal(const value_t& expr, environment_ptr env) {
    ...
    if (auto* sym = std::get_if<symbol>(&expr.v)) {
        if (sym->name == "nil") return nil_value;
        if (sym->name == "t")   return true_value;
        if (auto* val = env->lookup(sym->name)) return *val;
    ...
    if (!lp || !*lp) return expr;
    auto& elems = **lp;
    if (elems.empty()) return make_list();   // empty list literal
    ...
    // --- Special forms ---
    if (name == "if")       return do_if(args, env);
    if (name == "begin")    return do_begin(args, env);
    if (name == "while")    return do_while(args, env);
    if (name == "for")      return do_for(args, env);
    if (name == "set")      return do_set(args, env);
    if (name == "quote")    return do_quote(args);
    if (name == "lambda")   return do_lambda(args, env);
    if (name == "return")   return do_return(args, env);
    if (name == "let")      return do_let(args, env);
    if (name == "super")    return do_super(args, env);
    if (name == "defun")    return do_defun(args, env);
    if (name == "defclass") return do_defclass(args, env);
    if (name == "defmacro") return do_defmacro(args, env);
    if (name == "eval")     return do_eval(args, env);
    if (name == "root")     return do_root();
...
value_t evaluator::do_super(const std::vector<value_t>& args, environment_ptr env) {
    ...
    else method_name = std::get<std::string>(eval_internal(args[1], env).v);
...
value_t evaluator::do_defclass(const std::vector<value_t>& args, environment_ptr env) {
    ...
    for (size_t i = methods_start; i < args.size(); ++i) {
        ...
        auto cls_env = env;
        native_fn wrapper = [self, method_closure, cls_env]
    ...
    evaluator* ev = this;
    native_fn constructor = [ev, cls_dict, cls_dict_val]
...
value_t evaluator::apply_closure(const closure_ptr& cls,
