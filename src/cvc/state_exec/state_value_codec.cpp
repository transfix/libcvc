...
void set_type(cvc::state& n, const char* t) {
    n(TYPE_KEY).value(std::string(t));
}
...
bool has_child(cvc::state& n, const std::string& name) {
    return n.findDescendant(name) != nullptr;
}
...
std::string phase_to_string(eval_phase p) {
    switch (p) {
        case eval_phase::init:             return "init";
        case eval_phase::eval_args:        return "eval_args";
        case eval_phase::apply:            return "apply";
        case eval_phase::if_cond:          return "if_cond";
        case eval_phase::if_branch:        return "if_branch";
        case eval_phase::begin_next:       return "begin_next";
        case eval_phase::while_cond:       return "while_cond";
        case eval_phase::while_body:       return "while_body";
        case eval_phase::for_body:         return "for_body";
        case eval_phase::set_value:        return "set_value";
        case eval_phase::let_bindings:     return "let_bindings";
        case eval_phase::let_body:         return "let_body";
        case eval_phase::return_value:     return "return_value";
        case eval_phase::eval_inner:       return "eval_inner";
        case eval_phase::super_args:       return "super_args";
        case eval_phase::defclass_methods: return "defclass_methods";
    }
    return "init";
}

eval_phase string_to_phase(const std::string& s) {
    if (s == "eval_args")        return eval_phase::eval_args;
    if (s == "apply")            return eval_phase::apply;
    if (s == "if_cond")          return eval_phase::if_cond;
    if (s == "if_branch")        return eval_phase::if_branch;
    if (s == "begin_next")       return eval_phase::begin_next;
    if (s == "while_cond")       return eval_phase::while_cond;
    if (s == "while_body")       return eval_phase::while_body;
    if (s == "for_body")         return eval_phase::for_body;
    if (s == "set_value")        return eval_phase::set_value;
    if (s == "let_bindings")     return eval_phase::let_bindings;
    if (s == "let_body")         return eval_phase::let_body;
    if (s == "return_value")     return eval_phase::return_value;
    if (s == "eval_inner")       return eval_phase::eval_inner;
    if (s == "super_args")       return eval_phase::super_args;
    if (s == "defclass_methods") return eval_phase::defclass_methods;
