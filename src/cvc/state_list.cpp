...
state_list::state_list(state& node, std::size_t pad_width)
    : _node(node), _pad_width(pad_width)
{
...
std::size_t state_list::size() const
{
    return _node(len_key).value<std::size_t>();
}

bool state_list::empty() const
{
    return size() == 0;
}

void state_list::set_size(std::size_t n)
{
    _node(len_key).value(n);
}
