...
// Forward declarations from octree.cpp
extern int point_in_polygon(SDFContext* ctx, myPoint result, int tri);
extern int ray_polygon_intersection (SDFContext* ctx, ray r, int tri);
extern int confirm_SDF(int flag);

double point_2_plane(SDFContext* ctx, int tri, double i, double j, double k, myPoint* inter);
double getClipPoint(SDFContext* ctx, int one, int two, double i, double j, double k, myPoint* inter);

double getEuclideanDist(SDFContext* ctx, int on, double vi, double vj, double vk)
{
	myPoint one;

	one.x = ctx->vertices[on].x;		one.y = ctx->vertices[on].y;		one.z = ctx->vertices[on].z;
...
int chq_inters(SDFContext* ctx, int pts[100], int inters, double  vi, double vj, double vk)
{
	int i, j, counter, ret;
	myPoint interPoint;
	double distns[500];
	...
	for (counter=0,i=1; i<2*inters;)
	{
		...
		case 1: break;
		case 21: case -23:
			distns[counter] = getClipPoint(ctx, ctx->surface[pts[i-1]].v1, ctx->surface[pts[i-1]].v2, vi, vj, vk, &interPoint);
			counter++;
			break;
		case 22: case -22:
			distns[counter] = getClipPoint(ctx, ctx->surface[pts[i-1]].v2, ctx->surface[pts[i-1]].v3, vi, vj, vk, &interPoint);
			counter++;
			break;
		case 23: case -21:
			distns[counter] = getClipPoint(ctx, ctx->surface[pts[i-1]].v3, ctx->surface[pts[i-1]].v1, vi, vj, vk, &interPoint);
			counter++;
			break;
		case 31: case -33:
			distns[counter] = getEuclideanDist(ctx, ctx->surface[pts[i-1]].v3, vi, vj, vk);
			counter++;
			break;
		case 32: case -32:
			distns[counter] = getEuclideanDist(ctx, ctx->surface[pts[i-1]].v1, vi, vj, vk);
			counter++;
			break;
		case 33: case -31:
			distns[counter] = getEuclideanDist(ctx, ctx->surface[pts[i-1]].v2, vi, vj, vk);
			counter++;
			break;
		}
		i +=2;
...
int x_assign(SDFContext* ctx, int vi, int vj, int vk)
{
	...
	r.ox = ctx->xCoord(vi);		r.oy = ctx->yCoord(vj);		r.oz = ctx->zCoord(vk);
	...
	return (chq_inters(ctx, pts, inters, r.ox, r.oy, r.oz));
}
...
int klc_assign(SDFContext* ctx, int vi, int vj, int vk)
{
	int inters[3];

	if ((vi<=0) || (vj<=0) || (vk<=0) || (vi>=ctx->size) || (vj>=ctx->size) || (vk>=ctx->size))
	{
		return 1;
	}
	else
	{
		inters[0] = x_assign(ctx, vi, vj, vk);
		inters[1] = y_assign(ctx, vi, vj, vk);
		inters[2] = z_assign(ctx, vi, vj, vk);
	}
...
double sort_3_distances(SDFContext* ctx, double vals[3], myPoint closest[3], myPoint* inter)
{
	double dist;
...
double getClipPoint(SDFContext* ctx, int one, int two, double i, double j, double k, myPoint* inter)
{
	...
	d1 = ctx->vertices[one].x - ctx->vertices[two].x;
	d2 = ctx->vertices[one].y - ctx->vertices[two].y;
	d3 = ctx->vertices[one].z - ctx->vertices[two].z;
	denom = d1*d1 + d2*d2 + d3*d3;
    denom = sqrt(denom);
    tempLen = denom; //len of the tri edge
	d1 /=denom;	d2 /=denom;	d3 /=denom;

	e1 = i - ctx->vertices[two].x;
	e2 = j - ctx->vertices[two].y;
	e3 = k - ctx->vertices[two].z;
	denom = e1*e1 + e2*e2 + e3*e3;
	if (isZero(denom))
	{
		//Then, the pt is CLOSE to the vertex TWO.
		inter->x = ctx->vertices[two].x;		inter->y = ctx->vertices[two].y;		inter->z = ctx->vertices[two].z;
		return fabs(denom);
	}
	...
	if (isZero(theta))
	{
		d1 = i - ctx->vertices[one].x;
		d2 = j - ctx->vertices[one].y;
		d3 = k - ctx->vertices[one].z;
		theta = sqrt(d1*d1 + d2*d2 + d3*d3);
		
		if (theta <=denom)
		{
			inter->x = ctx->vertices[one].x;		inter->y = ctx->vertices[one].y;		inter->z = ctx->vertices[one].z;	return fabs(theta);
		}
		else
		{
			inter->x = ctx->vertices[two].x;		inter->y = ctx->vertices[two].y;		inter->z = ctx->vertices[two].z;	return fabs(denom);
		}		
	}
    ...
    else if ((denom * theta) > tempLen)
    {
		...
		inter->x = ctx->vertices[one].x;		inter->y = ctx->vertices[one].y;		inter->z = ctx->vertices[one].z;	
        return sqrt(theta);
    }
    else
    {
        t = denom*theta;				theta = acos(theta);		
		inter->x = ctx->vertices[two].x + t*(ctx->vertices[one].x - ctx->vertices[two].x);
		inter->y = ctx->vertices[two].y + t*(ctx->vertices[one].y - ctx->vertices[two].y);
		inter->z = ctx->vertices[two].z + t*(ctx->vertices[one].z - ctx->vertices[two].z);
...
double point_2_plane(SDFContext* ctx, int tri, double i, double j, double k, myPoint* inter)
{
	...
	dist = ( ((i* ctx->normals[tri].x +	j* ctx->normals[tri].y + k* ctx->normals[tri].z)) + ctx->distances[tri]);

	if (isZero(dist))
	{
		res.x =i;	res.y =j;	res.z =k;
		if(point_in_polygon(ctx, res, tri))
		{
	...
	res.x = (double)(i -ctx->normals[tri].x*dist);
	res.y = (double)(j -ctx->normals[tri].y*dist);
	res.z = (double)(k -ctx->normals[tri].z*dist);
	if (point_in_polygon(ctx, res, tri))
	{
		(*inter).x = res.x;		(*inter).y = res.y;			(*inter).z = res.z;
		return fabs(dist);
	...
	temp[0] = getClipPoint(ctx, ctx->surface[tri].v1, ctx->surface[tri].v2, i, j, k, &val[0]);
    temp[1] = getClipPoint(ctx, ctx->surface[tri].v3, ctx->surface[tri].v2, i, j, k, &val[1]);
    temp[2] = getClipPoint(ctx, ctx->surface[tri].v1, ctx->surface[tri].v3, i, j, k, &val[2]);

    dist = sort_3_distances(ctx, temp, val, inter);

	//if (dist >= (ctx->MAX_DIST) || dist <= (-1*ctx->MAX_DIST))
	//	printf("ERR: vert= <%lf %lf %lf> tri= <%d> dist is %lf, max allowed = %lf\n", i, j, k, tri, dist, ctx->MAX_DIST);
	return dist;
}
...
int each_cell(SDFContext* ctx, int ci, int cj, int ck, int vi, int vj, int vk)
{
	...
	//ctx->voxel_values[vert].processed =1;

	if (ctx->voxel_values[vert].value >= (ctx->MAX_DIST) || ctx->voxel_values[vert].value <= (-1*ctx->MAX_DIST))
...
void compute_SDF(SDFContext* ctx, int i, int j, int k)
{
	int level, ci, cj, ck;

	//Get the corresponding Octree cell.
	if (i == ctx->size) ci = i-1;	else	ci=i;
	if (j == ctx->size) cj = j-1;	else	cj=j;
	if (k == ctx->size) ck = k-1;	else	ck=k;
	...
	for (ci=i-level; ci<=i+level; ci++)
	{
		for (cj=j-level; cj<=j+level; cj++)
		{
			...
			{
				if ((ci < 0) || (ci >= ctx->size))  continue;
				if ((cj < 0) || (cj >= ctx->size))  continue;
				if ((ck < 0) || (ck >= ctx->size))  continue;

				if (ctx->sdf[ci][cj][ck].useful >0)
					each_cell(ctx, ci, cj, ck, i, j, k);
			}
		}
	}
}
...
void compute_boundarySDF(SDFContext* ctx)
{
	int ind, vi, vj, vk;

	for (ind = 0; ind < ctx->all_verts_touched; ind++)
	{
		ctx->vert2index(ctx->queues[ind], vi, vj, vk);
		compute_SDF(ctx, vi, vj, vk);
		ctx->voxel_values[ctx->queues[ind]].processed = 1;
		if (ind % 5000 == 0) 
			printf("computing the boundary SDF %3.4f %% over\n", (100.0 * ind / ctx->all_verts_touched));
	}
}

void compute_signs(SDFContext* ctx)
{
	int i, j, k, sgn, ind;
...
//		fprintf(fp, "%d\n", i);
		for(j=0; j<=ctx->size; j++)
		{
			...
			{
				sgn= klc_assign(ctx, i, j, k);
				ind = ctx->index2vert(i, j, k);
				ctx->voxel_values[ind].signe = sgn;
/*
				if (sgn== -1)
					fprintf(fp, "0");
				else
...
//			fprintf(fp, "\n");	
		...
		fprintf(stderr,"SDF computations: %5.2f %%\r",(float(i)/float(ctx->size))*100.0);
	}
//	fclose(fp);
}

// Legacy compute() function removed - use SDFContext::compute() instead
