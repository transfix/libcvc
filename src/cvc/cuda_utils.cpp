...
namespace CVC_NAMESPACE
{
  ...
  cuda_device_manager::cuda_device_manager() 
    : _cuda_available(false), _device_count(0) {
    initialize();
  }
  ...
  void cuda_device_manager::initialize() {
    ...
    for (int i = 0; i < _device_count; ++i) {
      ...
      if (cudaGetDeviceProperties(&prop, i) == cudaSuccess) {
        ...
        _devices[i].unified_memory_supported = (prop.major >= 3); // Unified memory requires compute capability 3.0+
  ...
  bool cuda_device_manager::cuda_available() {
    return instance()._cuda_available;
  }

  int cuda_device_manager::device_count() {
    return instance()._device_count;
  }

  std::vector<gpu_device_info> cuda_device_manager::get_device_info() {
  ...
  bool cuda_device_manager::can_access_peer(int device_id, int peer_device_id) {
    ...
    auto& inst = instance();
    if (device_id < 0 || device_id >= inst._device_count ||
        peer_device_id < 0 || peer_device_id >= inst._device_count) {
      return false;
    }
  ...
  void cuda_device_manager::enable_peer_access(int device_id, int peer_device_id) {
    ...
    if (!can_access_peer(device_id, peer_device_id)) {
      std::ostringstream oss;
      oss << "Cannot enable peer access from device " << device_id 
          << " to device " << peer_device_id;
      throw cuda_error(oss.str());
    }
