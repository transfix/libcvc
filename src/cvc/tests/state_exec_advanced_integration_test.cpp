...
static cvc::state_mutation make_set_value(const std::string& origin,
                                          uint64_t seq,
...
static cvc::state_mutation make_data_mutation(const std::string& origin,
                                              uint64_t seq,
                                              const std::string& path,
...
static const char* MIME_ELECTION =
    "application/x-state-exec-election";

/// Force a coordinator to become leader.
static void make_leader(exec_coordinator& coord,
                         const std::string& node_id,
                         const std::string& cluster_id) {
    auto victory = cvc::state_message::make_text(
        "__state_exec." + cluster_id + ".election",
        "{\"type\":\"election-victory\",\"node_id\":\"" + node_id + "\"}",
        MIME_ELECTION);
...
TEST_F(ContentionTest, TenNodesBurstWritesSamePath) {
    ...
    for (int seq = 1; seq <= kWritesPerNode; ++seq) {
        for (int i = 0; i < kNodes; ++i) {
            std::string origin = "node-" + std::string(1, 'A' + i);
            auto m = make_set_value(origin, static_cast<uint64_t>(seq),
                                    "hot.key", "v" + std::to_string(seq) + "-" + origin);
            auto r = shard->ingest_remote(m);
            if (r.applied) ++total_applied;
        }
    }
...
TEST_F(ContentionTest, ConcurrentThreadedContention) {
    ...
    auto worker = [&](int thread_id) {
        ...
        for (int s = 1; s <= kPerThread; ++s) {
            auto m = make_set_value(origin, static_cast<uint64_t>(s),
                                    "concurrent.hot", origin + ":" + std::to_string(s));
            auto r = shard->ingest_remote(m);
            if (r.applied) total_applied.fetch_add(1);
        }
    };
...
TEST_F(ClusterHierarchyTest, ChildClusterWritesToItsOwnSubtree) {
    ...
    cvc::state::instance(alpha_app)("scene.geometry.vertex_count")
        .value(std::string("init"));
    cvc::state::instance(alpha_app)("scene.geometry.vertex_count")
        .value(std::string("1024"));

    alpha_transport.pump_all();

    EXPECT_EQ(cvc::state::instance(alpha_app)("scene.geometry.vertex_count").value(),
              "1024");
}
...
TEST_F(ClusterHierarchyTest, TwoNodeChildClusterReplicates) {
    ...
    cvc::state::instance(alpha_app)("scene.mesh.triangles")
        .value(std::string("init"));
    cvc::state::instance(alpha_app)("scene.mesh.triangles")
        .value(std::string("5000"));

    // Pump replicates to node-2
    alpha_transport.pump_all();

    EXPECT_EQ(cvc::state::instance(alpha_app2)("scene.mesh.triangles").value(),
              "5000");
...
TEST_F(DelegationSecurityTest, DelegationWithLeaseExpiry) {
    ...
    shard.delegation().delegate("temp-data", "ephemeral-cluster",
                                "grpc://ephemeral:1234",
                                1000000);  // 1ms lease
...
TEST_F(MultiNodeReplicationTest, WriteOnNode1ReplicatesToAll) {
    // First value() on a new child is consumed by creation, so set twice.
    cvc::state::instance(app1)("shared.value")
        .value(std::string("init"));
    cvc::state::instance(app1)("shared.value")
        .value(std::string("from_node_1"));
    transport.pump_all();
...
TEST_F(MultiNodeReplicationTest, ConflictingWritesDeterministicWinner) {
    ...
    s1.ingest_remote(m1); s1.ingest_remote(m2); s1.ingest_remote(m3);
    s2.ingest_remote(m1); s2.ingest_remote(m2); s2.ingest_remote(m3);
    s3.ingest_remote(m1); s3.ingest_remote(m2); s3.ingest_remote(m3);
...
TEST_F(MultiNodeReplicationTest, SequentialOverwrites) {
    ...
    cvc::state::instance(app1)("seq.value")
        .value(std::string("init"));
    transport.pump_all();
...
TEST_F(CoordinatedMessagingTest, WorkerPoolWithMsgSend) {
    ...
    for (int i = 0; i < kWorkers; ++i) {
        std::string script =
            "(begin"
            "  (state-set \"worker." + std::to_string(i) + ".result\" "
            "    (str (* " + std::to_string(i + 1) + " " + std::to_string(i + 1) + ")))"
...
protected:
    ...
    void TearDown() override {
        if (coord.is_running()) coord.stop();
    }
};
