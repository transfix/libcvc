...
TEST(StateExecParserTest, UnclosedParen) {
    EXPECT_THROW(parse("(+ 1 2"), parse_error);
}

TEST(StateExecParserTest, UnterminatedString) {
    EXPECT_THROW(parse("\"hello"), parse_error);
}
...
TEST(StateExecParserTest, OperatorSymbols) {
    auto syms = {"<=", ">=", "!=", "str-concat", "get-attr",
                 "set-attr", "del-attr", "del-nth"};
    for (auto s : syms) {
        auto v = parse(s);
