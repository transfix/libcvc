...
TEST_F(StateTreeIntrinsicsTest, StateGetWrongArgCount) {
    ...
    EXPECT_THROW(call("state-get", {std::string("a"), std::string("b")}),
                 std::runtime_error);
}
...
TEST_F(ExpiryIntrinsicsTest, ExpireNonexistentThrows) {
    EXPECT_THROW(call("state-expire", {std::string("no.path"), 100.0}),
                 std::runtime_error);
}
...
TEST_F(MsgSendIntrinsicsTest, SendWithContentType) {
    auto result = call("msg-send",
        {std::string("msg.typed"), std::string("{\"a\":1}"), std::string("application/json")});
    auto* d = std::get_if<dict_ptr>(&result.v);
    ASSERT_NE(d, nullptr);
}
...
TEST_F(RegisterIntrinsicsTest, AllStateIntrinsicsRegistered) {
    for (auto& name : {"state-get", "state-set", "state-children",
                       "state-exists", "state-delete",
                       "state-data-get", "state-data-set"}) {
        EXPECT_NE(env->lookup(name), nullptr)
            << "Missing intrinsic: " << name;
    }
}

TEST_F(RegisterIntrinsicsTest, AllSchedulerIntrinsicsRegistered) {
    for (auto& name : {"spawn", "fork", "self-pid", "self-uid",
                       "kill", "pause", "resume", "ps", "inspect"}) {
        EXPECT_NE(env->lookup(name), nullptr)
            << "Missing intrinsic: " << name;
    }
}
...
TEST_F(RegisterIntrinsicsTest, AllResourceIntrinsicsRegistered) {
    for (auto& name : {"memory-usage", "memory-limit",
                       "time-elapsed", "time-limit",
                       "message-count", "message-limit", "step-count"}) {
        EXPECT_NE(env->lookup(name), nullptr)
            << "Missing intrinsic: " << name;
    }
}
...
TEST_F(RegisterIntrinsicsTest, AllExpiryIntrinsicsRegistered) {
    for (auto& name : {"state-expire", "state-expire-at",
...
TEST_F(StdlibRegistryTest, ImportUnknownFunctionThrows) {
    auto env = builtins::make_default_environment();
    EXPECT_THROW(registry.import_module("string", env, {"no.such.fn"}),
                 std::runtime_error);
}
...
TEST_F(StdlibStringTest, CharAtOutOfRange) {
    EXPECT_THROW(call("string.char-at", {std::string("hi"), int64_t(5)}),
                 std::runtime_error);
}
...
TEST_F(StdlibCollectionsTest, RangeZeroStepThrows) {
    EXPECT_THROW(call("collections.range", {int64_t(0), int64_t(5), int64_t(0)}),
                 std::runtime_error);
}
...
TEST_F(StdlibCollectionsTest, DictMerge) {
    ...
    for (auto& [k, v] : **r) {
        if (k == "b") { EXPECT_EQ(std::get<int64_t>(v.v), 99); found_b = true; }
        if (k == "c") { EXPECT_EQ(std::get<int64_t>(v.v), 3); found_c = true; }
    }
    EXPECT_TRUE(found_b);
    EXPECT_TRUE(found_c);
}
