...
void test_sphere()
{
  ...
  int expectedTris = thetaRes + (phiRes - 2) * thetaRes * 2 + thetaRes; // top cap + middle + bottom cap
...
void test_cube()
{
  ...
  for (const auto& n : cube.normals()) {
    ...
    int nonzero = 0;
    if (std::abs(std::abs(n[0]) - 1.0) < tolerance) nonzero++;
    if (std::abs(std::abs(n[1]) - 1.0) < tolerance) nonzero++;
    if (std::abs(std::abs(n[2]) - 1.0) < tolerance) nonzero++;
    assert(nonzero == 1);
  }
...
void test_transforms()
{
  ...
  std::cout << "Sphere bbox: [" << bbox[0] << ", " << bbox[3] << "] x ["
            << bbox[1] << ", " << bbox[4] << "] y ["
  ...
  bbox = cube.extents();
  std::cout << "Cube bbox: [" << bbox[0] << ", " << bbox[3] << "] x ["
            << bbox[1] << ", " << bbox[4] << "] y ["
            << bbox[2] << ", " << bbox[5] << "]" << std::endl;
