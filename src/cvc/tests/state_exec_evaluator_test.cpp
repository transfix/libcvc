...
protected:
    ...
    void SetUp() override {
        ev = std::make_unique<evaluator>(builtins::make_default_environment());
    }

    value_t eval(const std::string& script) {
        return ev->evaluate_script(script);
    }
...
TEST_F(EvaluatorTest, IntegerLiteral) {
    EXPECT_EQ(eval_int("42"), 42);
}

TEST_F(EvaluatorTest, DoubleLiteral) {
    EXPECT_DOUBLE_EQ(eval_double("3.14"), 3.14);
}

TEST_F(EvaluatorTest, StringLiteral) {
    EXPECT_EQ(eval_string("\"hello\""), "hello");
}

TEST_F(EvaluatorTest, NilLiteral) {
    EXPECT_TRUE(eval("nil").is_nil());
}

TEST_F(EvaluatorTest, TrueLiteral) {
    EXPECT_TRUE(eval_bool("t"));
}

// ─── Arithmetic ────────────────────────────────────────────────────────────

TEST_F(EvaluatorTest, Addition) {
    EXPECT_EQ(eval_int("(+ 1 2 3)"), 6);
}

TEST_F(EvaluatorTest, Subtraction) {
    EXPECT_EQ(eval_int("(- 10 3 2)"), 5);
}

TEST_F(EvaluatorTest, Multiplication) {
    EXPECT_EQ(eval_int("(* 3 4)"), 12);
}

TEST_F(EvaluatorTest, Division) {
    EXPECT_DOUBLE_EQ(eval_double("(/ 10 4)"), 2.5);
}

TEST_F(EvaluatorTest, Modulo) {
    EXPECT_EQ(eval_int("(% 10 3)"), 1);
}
...
TEST_F(EvaluatorTest, IfNoElse) {
    EXPECT_TRUE(eval("(if nil 1)").is_nil());
}

TEST_F(EvaluatorTest, Begin) {
    EXPECT_EQ(eval_int("(begin 1 2 3)"), 3);
}

TEST_F(EvaluatorTest, BeginEmpty) {
    EXPECT_TRUE(eval("(begin)").is_nil());
}

TEST_F(EvaluatorTest, Set) {
...
TEST_F(EvaluatorTest, ListLength) {
    EXPECT_EQ(eval_int("(length (list 1 2 3))"), 3);
}
