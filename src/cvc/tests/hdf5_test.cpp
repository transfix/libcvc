...
class HDF5Test : public ::testing::Test {
 protected:
  ...
  virtual void SetUp() {
    ...
    pid_t pid = getpid();
    test_dir = std::string(getenv("CMAKE_CURRENT_BINARY_DIR") ? 
                           getenv("CMAKE_CURRENT_BINARY_DIR") : ".") +
...
TEST_F(HDF5Test, GetGroupCreate) {
  ...
  // Create a group
  boost::shared_ptr<H5::Group> group = 
...
TEST_F(HDF5Test, GetGroupNested) {
  ...
  boost::shared_ptr<H5::Group> group = 
    hdf5_utils::getGroup(*file, "level1/level2/level3", true);
  EXPECT_TRUE(group.get() != NULL);
}
...
TEST_F(HDF5Test, GetGroupOpenExisting) {
  ...
  // Reopen and verify group exists
  {
    boost::shared_ptr<H5::H5File> file = hdf5_utils::getH5File(filepath, false);
    boost::shared_ptr<H5::Group> group = 
      hdf5_utils::getGroup(*file, "mygroup", false);
    EXPECT_TRUE(group.get() != NULL);
  }
}
...
TEST_F(HDF5Test, GetDataSetCreateNested) {
  ...
  boost::shared_ptr<H5::Group> group = 
    hdf5_utils::getGroup(*file, "group1/group2", true);
...
TEST_F(HDF5Test, HasAttributeTrue) {
  ...
  H5::Attribute attr = file->createAttribute("test_attr", H5::PredType::NATIVE_DOUBLE,
...
TEST_F(HDF5Test, UnlinkNestedObject) {
  ...
  
  {
    ...
    boost::shared_ptr<H5::Group> grp = 
      hdf5_utils::getGroup(*file, "group/subgroup", true);
...
TEST_F(HDF5Test, MultipleGroupsAndDataSets) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    boost::shared_ptr<H5::Group> group = 
      hdf5_utils::getGroup(*file, group_path, true);
...
TEST_F(HDF5Test, MixedAttributes) {
  ...
  H5::Attribute attr_d = dataset.createAttribute("pi", H5::PredType::NATIVE_DOUBLE,
                                                  H5::DataSpace(H5S_SCALAR));
  attr_d.write(H5::PredType::NATIVE_DOUBLE, &dval);
  
  H5::Attribute attr_u = dataset.createAttribute("answer", H5::PredType::NATIVE_UINT,
                                                  H5::DataSpace(H5S_SCALAR));
...
TEST_F(HDF5Test, CreateModifyRead) {
  ...
  // Create and write
  {
    ...
    boost::shared_ptr<H5::Group> group = 
      hdf5_utils::getGroup(*file, "data", true);
  ...
  // Reopen and verify
  {
    ...
    boost::shared_ptr<H5::DataSet> dataset = 
      hdf5_utils::getDataSet(*file, "data/values", false);
