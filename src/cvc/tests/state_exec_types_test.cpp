...
TEST(StateExecTypesTest, ListValue) {
    auto list = make_list({value_t(int64_t(1)), value_t(int64_t(2)),
                           value_t(int64_t(3))});
...
TEST(StateExecTypesTest, DictValue) {
    auto dict = make_dict({{"name", value_t("Alice")},
                           {"age", value_t(int64_t(30))}});
...
TEST(StateExecTypesTest, NativeFnValue) {
    native_fn fn = [](std::span<const value_t>) -> value_t {
        return value_t(int64_t(42));
    };
    value_t v(fn);
