...
TEST_F(CoordinatorTest, SingleNodeBecomesLeader) {
    ...
    auto hb = state_message::make_text(
        "__state_exec.cluster-1.heartbeat",
...
TEST_F(CoordinatorTest, ElectionBullyLowerIdLoses) {
    ...
    bus.subscribe("", [&](const state_message& m) {
...
TEST_F(CoordinatorTest, Handoff) {
    ...
    auto victory = state_message::make_text(
        "__state_exec.cluster-1.election",
...
TEST_F(CoordinatorTest, HeartbeatEmitAndReceive) {
    ...
    auto hb = state_message::make_text(
        "__state_exec.cluster-1.heartbeat",
        "{\"node_id\":\"node-B\",\"is_leader\":false,\"stats\":\"{\\\"total_processes\\\":3,\\\"running\\\":2,\\\"ready\\\":1,\\\"paused\\\":0,\\\"terminated\\\":0,\\\"killed\\\":0,\\\"total_steps\\\":100}\"}",
        MIME_EXEC_HEARTBEAT);
    hb.cluster_id = "cluster-1";
...
TEST_F(CoordinatorTest, MigrateRunningProcess) {
    ...
    std::vector<state_message> captured;
    bus.subscribe("__state_exec.cluster-1.migrate", [&](const state_message& m) {
        captured.push_back(m);
    });
...
TEST_F(CoordinatorTest, PsAllWithRemote) {
    ...
    for (const auto& p : procs) {
        if (p.node_id == "node-A") found_local = true;
        if (p.node_id == "node-B") found_remote = true;
    }
...
TEST_F(CoordinatorTest, AdminControlRemote) {
    ...
    std::vector<state_message> captured;
    bus.subscribe("__state_exec.cluster-1.control", [&](const state_message& m) {
        captured.push_back(m);
    });
...
TEST_F(CoordinatorTest, E2E_MultiNodeHeartbeatAggregation) {
    ...
    for (int i = 1; i <= 3; ++i) {
        ...
        stats_json << "{\\\"total_processes\\\":" << i * 2
                   << ",\\\"running\\\":" << i
                   << ",\\\"ready\\\":" << i
                   << ",\\\"paused\\\":0,\\\"terminated\\\":0"
                   << ",\\\"killed\\\":0,\\\"total_steps\\\":" << i * 100
                   << "}";

        auto hb = state_message::make_text(
            "__state_exec.cluster-1.heartbeat",
            "{\"node_id\":\"" + nid + "\",\"is_leader\":false,"
...
protected:
    ...
    void TearDown() override {
        if (coord1.is_running()) coord1.stop();
        if (coord2.is_running()) coord2.stop();
    }
};
...
TEST_F(TwoCoordinatorTest, CrossNodeControlCommands) {
    ...
    auto ctrl = state_message::make_text(
        "__state_exec.cluster-1.control.node-A",
