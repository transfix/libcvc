...
TEST_F(BuiltinsTest, AddEmpty) {
    EXPECT_EQ(std::get<int64_t>(call("+", {}).v), 0);
}
...
TEST_F(BuiltinsTest, SubUnary) {
    EXPECT_EQ(std::get<int64_t>(call("-", {int64_t{7}}).v), -7);
}
...
TEST_F(BuiltinsTest, StrConcat) {
    EXPECT_EQ(std::get<std::string>(call("str-concat",
        {std::string("hello"), std::string(" "), std::string("world")}).v),
        "hello world");
}

TEST_F(BuiltinsTest, Str) {
    EXPECT_EQ(std::get<std::string>(call("str", {int64_t{42}}).v), "42");
}
...
TEST_F(BuiltinsTest, Slice) {
    auto lst = make_list({value_t{int64_t{10}}, value_t{int64_t{20}},
                          value_t{int64_t{30}}, value_t{int64_t{40}}});
...
TEST_F(BuiltinsTest, DictCreate) {
    auto r = call("dict", {std::string("a"), int64_t{1},
                           std::string("b"), int64_t{2}});
...
TEST_F(BuiltinsTest, DelAttr) {
    auto obj = call("dict", {std::string("a"), int64_t{1},
                             std::string("b"), int64_t{2}});
...
TEST_F(BuiltinsTest, DefaultEnvironmentHasAllBuiltins) {
    std::vector<std::string> expected = {
        ...
        "<", ">", "<=", ">=", "=", "!=",
        "str-concat", "str",
        "list", "car", "cdr", "cons", "nth", "set-nth",
        "length", "append", "slice", "del-nth",
        "dict", "get-attr", "set-attr", "del-attr",
        "send", "apply",
        "is-null", "is-list", "type-of",
        "print",
        "not", "and", "or",
    };
