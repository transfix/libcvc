...
static task<void> void_coro(bool* called) { *called = true; co_return; }
...
protected:
    ...
    void SetUp() override {
        ev = std::make_unique<async_evaluator>(
            builtins::make_default_environment());
    }

    value_t eval(const std::string& script) {
        return ev->sync_evaluate_script(script);
    }

    int64_t eval_int(const std::string& s) {
        return std::get<int64_t>(eval(s).v);
    }

    double eval_double(const std::string& s) {
        return std::get<double>(eval(s).v);
    }

    bool eval_bool(const std::string& s) {
        return std::get<bool>(eval(s).v);
    }

    std::string eval_string(const std::string& s) {
        return std::get<std::string>(eval(s).v);
    }
};

// --- Atoms ---

TEST_F(AsyncEvaluatorTest, IntegerLiteral) {
    EXPECT_EQ(eval_int("42"), 42);
}

TEST_F(AsyncEvaluatorTest, DoubleLiteral) {
    EXPECT_DOUBLE_EQ(eval_double("3.14"), 3.14);
}

TEST_F(AsyncEvaluatorTest, StringLiteral) {
    EXPECT_EQ(eval_string("\"hello\""), "hello");
}
...
TEST_F(AsyncEvaluatorTest, TrueLiteral) {
    EXPECT_TRUE(eval_bool("t"));
}

// --- Arithmetic ---

TEST_F(AsyncEvaluatorTest, Addition) {
    EXPECT_EQ(eval_int("(+ 1 2 3)"), 6);
}

TEST_F(AsyncEvaluatorTest, Subtraction) {
    EXPECT_EQ(eval_int("(- 10 3)"), 7);
}

TEST_F(AsyncEvaluatorTest, Multiplication) {
    EXPECT_EQ(eval_int("(* 4 5)"), 20);
}
...
TEST_F(AsyncEvaluatorTest, NestedArithmetic) {
    EXPECT_EQ(eval_int("(+ (* 2 3) (- 10 4))"), 12);
}
...
TEST_F(AsyncEvaluatorTest, IfTrue) {
    EXPECT_EQ(eval_int("(if t 1 2)"), 1);
}

TEST_F(AsyncEvaluatorTest, IfFalse) {
    EXPECT_EQ(eval_int("(if nil 1 2)"), 2);
}

TEST_F(AsyncEvaluatorTest, Begin) {
    EXPECT_EQ(eval_int("(begin 1 2 3)"), 3);
}

TEST_F(AsyncEvaluatorTest, SetAndRead) {
    EXPECT_EQ(eval_int("(begin (set x 42) x)"), 42);
}
...
TEST_F(AsyncEvaluatorTest, LetBasic) {
    EXPECT_EQ(eval_int("(let ((x 10) (y 20)) (+ x y))"), 30);
}
...
TEST_F(AsyncEvaluatorTest, Return) {
    EXPECT_EQ(eval_int(
        "(begin (set f (lambda () (begin (return 42) 99))) (f))"), 42);
}
...
TEST_F(AsyncEvaluatorTest, Eval) {
    EXPECT_EQ(eval_int("(eval (quote (+ 1 2)))"), 3);
}

// --- String ops ---

TEST_F(AsyncEvaluatorTest, StrConcat) {
    EXPECT_EQ(eval_string("(str-concat \"hello\" \" \" \"world\")"),
              "hello world");
}
...
TEST_F(AsyncEvaluatorTest, ListLength) {
    EXPECT_EQ(eval_int("(length (list 1 2 3 4 5))"), 5);
}
...
protected:
    ...
    void SetUp() override {
        ev = std::make_unique<async_stackless_evaluator>(
            builtins::make_default_environment());
    }
...
TEST_F(AsyncStacklessTest, IntegerLiteral) {
    EXPECT_EQ(eval_int("42"), 42);
}

TEST_F(AsyncStacklessTest, DoubleLiteral) {
    EXPECT_DOUBLE_EQ(eval_double("3.14"), 3.14);
}

TEST_F(AsyncStacklessTest, StringLiteral) {
    EXPECT_EQ(eval_string("\"hello\""), "hello");
}
...
TEST_F(AsyncStacklessTest, TrueLiteral) {
    EXPECT_TRUE(eval_bool("t"));
}

// --- Arithmetic ---

TEST_F(AsyncStacklessTest, Addition) {
    EXPECT_EQ(eval_int("(+ 1 2 3)"), 6);
}

TEST_F(AsyncStacklessTest, NestedArithmetic) {
    EXPECT_EQ(eval_int("(+ (* 2 3) (- 10 4))"), 12);
}

// --- Control flow ---

TEST_F(AsyncStacklessTest, IfTrue) {
    EXPECT_EQ(eval_int("(if t 1 2)"), 1);
}

TEST_F(AsyncStacklessTest, Begin) {
    EXPECT_EQ(eval_int("(begin 1 2 3)"), 3);
}

TEST_F(AsyncStacklessTest, SetAndRead) {
    EXPECT_EQ(eval_int("(begin (set x 42) x)"), 42);
}
...
TEST(AsyncCrossEvaluatorTest, SyncVsAsyncRecursive) {
    auto sync_ev = std::make_unique<evaluator>(
        builtins::make_default_environment());
    auto async_ev = std::make_unique<async_evaluator>(
        builtins::make_default_environment());
...
TEST(AsyncCrossEvaluatorTest, StacklessVsAsyncStackless) {
    auto sync_ev = std::make_unique<stackless_evaluator>(
        builtins::make_default_environment());
    auto async_ev = std::make_unique<async_stackless_evaluator>(
        builtins::make_default_environment());

    auto programs = {
        "42", "3.14", "\"hello\"", "nil", "t",
        "(+ 1 2 3)", "(- 10 3)", "(* 4 5)",
        "(if t 1 2)", "(begin 1 2 3)",
        "(begin (set x 42) x)",
