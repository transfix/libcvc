...
TEST_F(SchedulerTest, PauseNonexistent) {
...
TEST_F(SchedulerTest, KillNonexistent) {
...
TEST_F(SchedulerTest, MaxStepsLimitError) {
    ...
    auto proc_it = std::find_if(it.begin(), it.end(),
        [pid](const process_info& pi) { return pi.pid == pid; });
...
TEST_F(SchedulerTest, SignalDelivery) {
    ...
    execute_options opts;
    opts.signal_handlers["SIGUSR1"] = make_list({
        value_t{symbol{"begin"}},
        value_t{int64_t(42)}
    });
...
TEST_F(SchedulerTest, ForkNonexistentFails) {
...
TEST_F(CrossSchedulerTest, MultipleProgramsSameResults) {
    ...
    for (size_t i = 0; i < scripts.size(); ++i) {
        ...
        EXPECT_TRUE(values_equal(sv, av))
            << "Mismatch on script: " << scripts[i];
    }
}
