...
voxels &voxels::bilateralFilter(double radiometricSigma, double spatialSigma,
  ...
  for (c = 0; c < 256; c++) {
    ...
    radiometricTable[c] =
        exp((double)(factor * factor) / (-radiometricSigma * radiometricSigma * 2.0));
  }
  ...
  if (_using_cuda && _cuda_unified_ptr) {
    try {
      ...
      cuda_bilateral_filter(temp._cuda_unified_ptr.get(),   // source data
                            result._cuda_unified_ptr.get(), // destination data
                            XDim(), YDim(), ZDim(), radiometricSigma, spatialSigma, filterRadius,
                            valueRange, voxelType());
      ...
      _ctx.log(2,
               std::string("CUDA bilateral filter failed: ") + e.what() + ", falling back to CPU");
    }
  }
#endif
...
#ifdef _OPENMP
#pragma omp parallel for private(i, j, x, y, z, fsample, sum, denom, normalizedDiff, weight,       \
                                     bool1, bool2) schedule(dynamic)
#endif
      ...
      for (i = 0; i < int(XDim()); i++) {
        ...
        for (z = 0; z < filterDiameter; z++) {
          ...
          for (y = 0; y < filterDiameter; y++) {
            bool2 =
                bool1 && (j + y >= int(filterRadius) && j + y < int(YDim()) + int(filterRadius));
            for (x = 0; x < filterDiameter; x++) {
              if (i + x >= int(filterRadius) && i + x < int(XDim()) + int(filterRadius) && bool2) {
                normalizedDiff = fabs(fsample - temp(i + x - filterRadius, j + y - filterRadius,
                                                     k + z - filterRadius));
                ...
                denom += weight;
                sum +=
                    weight * temp(i + x - filterRadius, j + y - filterRadius, k + z - filterRadius);
              }
            }
          }
        }
