...
volume &volume::sub(uint64 off_x, uint64 off_y, uint64 off_z, const dimension &subvoldim
#ifdef _MSC_VER
                    ,
...
volume &volume::sub(const bounding_box &subvolbox, const dimension &subvoldim) {
  ...
  if (!subvolbox.isWithin(boundingBox()))
    throw sub_volume_out_of_bounds(
        "Subvolume bounding box must be within the bounding box of the original volume.");

  volume subvol(_ctx, subvoldim, voxelType(), subvolbox);
  ...
  for (k = 0; k < subvol.ZDim(); k++) {
    for (j = 0; j < subvol.YDim(); j++)
      for (i = 0; i < subvol.XDim(); i++) {
        try {
          subvol(i, j, k,
                 this->interpolate(subvolbox.minx + i * subvol.XSpan(),
          ...
          //               fprintf(stderr,"Volume::sub(): caught index_out_of_bounds exception
          //               (%s)."
...
volume &volume::combineWith(const volume &vol) { return combineWith(vol, voxel_dimensions()); }

// 12/29/2013 -- Joe R. -- Creation.
volume &volume::read(const std::string &filename, unsigned int var, unsigned int time,
...
volume &volume::write(const std::string &filename) {
  createVolumeFile(ctx(), filename, boundingBox(), voxel_dimensions(),
