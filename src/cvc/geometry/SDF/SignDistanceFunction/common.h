...
// Legacy global API (deprecated - use SDFContext directly for thread safety)
// These functions maintain backward compatibility but use a thread-local context
bool initSDF();
void readGeom(int nverts, float* verts, int ntris, int* tris);
void adjustData();
void compute();

// Utility functions (now available as static methods in SDFContext)
