...
geometry::geometry(app &ctx) : _geom_type(SURFACE_TRI), _extents_set(false), _ctx(&ctx) {
  for (uint64_t i = 0; i < 3; i++)
    _min[i] = _max[i] = 0.0;
...
bounding_box geometry::extents() const {
  ...
  return bounding_box(min_pt[0], min_pt[1], min_pt[2], max_pt[0], max_pt[1], max_pt[2]);
}
...
uint64_t geometry::num_lines() const { return _lines ? _lines->size() : 0; }

uint64_t geometry::num_tris() const { return _tris ? _tris->size() : 0; }

uint64_t geometry::num_quads() const { return _quads ? _quads->size() : 0; }

uint64_t geometry::num_tets() const { return _tets ? _tets->size() : 0; }

uint64_t geometry::num_hexs() const { return _hexs ? _hexs->size() : 0; }
...
geometry &geometry::merge(const geometry &geom) {
  ...
  tris().insert(tris().end(), geom.tris().begin(), geom.tris().end());
  {
    ...
    while (i != tris().end()) {
      for (tri_t::iterator j = i->begin(); j != i->end(); j++)
        *j += points().size() - geom.points().size();
      i++;
    }
  }
...
geometry geometry::tri_surface() const {
  ...
  if (boundary().size() != points().size()) {
    ...
    for (tris_t::const_iterator i = tris().begin(); i != tris().end(); i++)
...
geometry &geometry::calculate_surf_normals() {
  ...
  for (tris_t::const_iterator i = tri_geom.const_tris().begin(); i != tri_geom.const_tris().end();
       i++)
    for (tri_t::const_iterator j = i->begin(); j != i->end(); j++)
      neighbor_tris[tri_geom.const_points()[*j]].push_back(
          distance(tri_geom.const_tris().begin(), i));

  // now iterate over our points, and for each point push back a new normal vector
  normals().clear();
  for (points_t::const_iterator i = const_points().begin(); i != const_points().end(); i++) {
    vector_t norm = {{0.0, 0.0, 0.0}};
    list<unsigned int> &neighbors = neighbor_tris[*i];
    for (list<unsigned int>::iterator j = neighbors.begin(); j != neighbors.end(); j++) {
      vector_t cur_tri_norm;
      const tri_t &tri = tri_geom.const_tris()[*j];
      point_t p[3] = {tri_geom.const_points()[tri[0]], tri_geom.const_points()[tri[1]],
                      tri_geom.const_points()[tri[2]]};
      vector_t v1 = {{p[1][0] - p[0][0], p[1][1] - p[0][1], p[1][2] - p[0][2]}};
      vector_t v2 = {{p[2][0] - p[0][0], p[2][1] - p[0][1], p[2][2] - p[0][2]}};
...
geometry &geometry::compute_normals() {
  ...
  for (size_t i = 0; i < const_quads().size(); ++i) {
    ...
    if (quad[0] >= num_points() || quad[1] >= num_points() || quad[2] >= num_points() ||
        quad[3] >= num_points())
      continue;
    ...
    vector_t avg_normal = {{(normal1[0] + normal2[0]) * 0.5, (normal1[1] + normal2[1]) * 0.5,
...
geometry geometry::generate_wire_interior() const {
  ...
  if (!boundary().empty() && boundary().size() == points().size()) {
    ...
    if (!tris().empty()) {
      new_geom.tris().clear();
      for (tris_t::const_iterator j = tris().begin(); j != tris().end(); j++) {
...
geometry &geometry::reorient() {
  ...
  if (!tris().empty()) {
    // find all the triangles that each point is a part of
    map<point_t, list<unsigned int>> neighbor_tris;
    for (tris_t::iterator i = tris().begin(); i != tris().end(); i++)
      for (tri_t::iterator j = i->begin(); j != i->end(); j++)
    ...
    */
    ...
    for (points_t::iterator i = points().begin(); i != points().end(); i++) {
      ...
      for (list<unsigned int>::iterator j = neighbors.begin(); j != neighbors.end(); j++) {
        tri_t &tri = tris()[*j];
        for (int k = 0; k < 3; k++)
          if (dot(normals()[tri[k]], normals()[distance(points().begin(), i)]) < 0)
            for (int l = 0; l < 3; l++)
              normals()[tri[k]][l] *= -1.0;
      }
    }
...
void geometry::write(const std::string &filename) const { write_geometry(*this, filename); }
...
geometry::geometry(app &ctx, const std::string &filename)
    : _geom_type(SURFACE_TRI), _extents_set(false), _ctx(&ctx) {
  for (uint64_t i = 0; i < 3; i++)
    _min[i] = _max[i] = 0.0;
  init_ptrs();
  read(filename);
