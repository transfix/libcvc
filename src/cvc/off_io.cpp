...
struct off_io : public geometry_file_io {
  ...
  off_io() : _id("off_io : v1.0") { _extensions.push_back(".off"); }
  ...
  virtual geometry read(const std::string &filename) const {
    ...
    if (split_line.size() != 3)
      throw read_error(
          str(boost::format("Not an OFF file (wrong number of tokens in line 2: [%1%])") %
              split_line.size()));
  ...
  virtual void write(const geometry &geom, const std::string &filename) const {
    ...
    if (!out)
      throw write_error(string(BOOST_CURRENT_FUNCTION) + string(": Could not open ") + filename);
    ...
    for (uint64 i = 0; i < geom.num_points(); i++)
      out << geom.const_points()[i][0] << " " << geom.const_points()[i][1] << " "
          << geom.const_points()[i][2] << "\n";

    for (uint64 i = 0; i < geom.num_tris(); i++)
      out << "3 " << geom.const_tris()[i][0] << " " << geom.const_tris()[i][1] << " "
          << geom.const_tris()[i][2] << "\n";
  }
