...
cvc::volume sdf_library(cvc::app &ctx, const cvc::geometry &geom,
  ...
  float mins[3] = {static_cast<float>(bbox[0]), static_cast<float>(bbox[1]),
                   static_cast<float>(bbox[2])};
  float maxs[3] = {static_cast<float>(bbox[3]), static_cast<float>(bbox[4]),
  ...
  std::unique_ptr<float[]> values;
  {
    ...
    values = SDFLibrary::computeSDF_MT(geom.num_points(), v.get(), geom.num_tris(), t.get(),
                                       static_cast<int>(size), flipNormalsInt, mins, maxs);
    if (!values)
      throw sign_distance_function_error("SDFLibrary::computeSDF_MT() failed");
  }
...
cvc::volume sdf_library_v2(cvc::app &ctx, const cvc::geometry &geom,
  ...
  FaceVertSet3D fvs(geom.num_points(), geom.num_tris(), verts_vec.data(), tris_vec.data(), 0);
  ...
  int dims[3] = {static_cast<int>(dim.xdim), static_cast<int>(dim.ydim),
                 static_cast<int>(dim.zdim)};

  // Use new constructor with user-specified center to respect arbitrary bbox
  DistanceTransform dt(ctx, fvs, dims, bbox_center, 0.5f, scale_factors[0], scale_factors[1],
                       scale_factors[2]);
...
// ---- Change History ----
// 01/10/2014 -- Joe R. -- Creation.
template <class T, class A> bool get_arg(T &lhs, A &argv, const std::string &var) {
...
cvc::geometry convert(const LBIE::geoframe &geo) {
  ...
  if (geo.mesh_type == LBIE::geoframe::TETRA || geo.mesh_type == LBIE::geoframe::TETRA2) {
    ...
    copy(geo.triangles.begin(), geo.triangles.end(), ret_geom.tris().begin());
...
LBIE::geoframe convert(const cvc::geometry &geo) {
  ...
  copy(geo.points().begin(), geo.points().end(), ret_geom.verts.begin());
  ret_geom.normals.resize(geo.num_points());
  copy(geo.normals().begin(), geo.normals().end(), ret_geom.normals.begin());
  ret_geom.color.resize(geo.num_points());
  copy(geo.colors().begin(), geo.colors().end(), ret_geom.color.begin());
  ...
  if (has_tets && !has_hexs && !has_tris && !has_quads) {
    ...
    copy(geo.tris().begin(), geo.tris().end(), ret_geom.triangles.begin());
...
cvc::geometry cvc_mesher(const cvc::volume &vol, Arguments argv) {
  ...
  case BSPLINE_CONVOLUTION:
    normaltype_lbie = LBIE::Mesher::BSPLINE_CONVOLUTION;
    break;
  case CENTRAL_DIFFERENCE:
    normaltype_lbie = LBIE::Mesher::CENTRAL_DIFFERENCE;
    break;
  case BSPLINE_INTERPOLATION:
    normaltype_lbie = LBIE::Mesher::BSPLINE_INTERPOLATION;
    break;
  default:
    normaltype_lbie = LBIE::Mesher::BSPLINE_CONVOLUTION;
    break;
  ...
  return LBIE::do_mesh_geometry(vol, isovalue, isovalue_in, err, err_in, geom_type,
                                improvement_method_enum, normaltype_lbie, extraction_method_enum,
                                improve_iterations, dual_contouring, false, propertyVol);
}
...
// 12/27/2025 -- Joe R. -- Added flipNormals parameter.
volume sdf(app &ctx, const geometry &geom, const dimension &dim, const bounding_box &bbox_in,
           sdf_algorithm algorithm, bool flipNormals) {
...
geometry::tets_t decode_tets_from_triangles(const geometry::tris_t &encoded_tris) {
  ...
  for (size_t i = 0; i < num_tets; i++) {
    ...
    vertices.insert(tri0[0]);
    vertices.insert(tri0[1]);
    vertices.insert(tri0[2]);
    vertices.insert(tri1[0]);
    vertices.insert(tri1[1]);
    vertices.insert(tri1[2]);
    vertices.insert(tri2[0]);
    vertices.insert(tri2[1]);
    vertices.insert(tri2[2]);
    vertices.insert(tri3[0]);
    vertices.insert(tri3[1]);
    vertices.insert(tri3[2]);
...
std::array<double, 4> tet_barycentric(const geometry::point_t &p, const geometry::point_t &v0,
  ...
  auto tet_volume = [](const geometry::point_t &a, const geometry::point_t &b,
                       const geometry::point_t &c, const geometry::point_t &d) -> double {
    ...
    double cross[3] = {ac[1] * ad[2] - ac[2] * ad[1], ac[2] * ad[0] - ac[0] * ad[2],
                       ac[0] * ad[1] - ac[1] * ad[0]};
...
double interpolate_in_tet(const geometry::point_t &p, const geometry::tet_t &tet,
  ...
  auto weights =
      tet_barycentric(p, vertices[tet[0]], vertices[tet[1]], vertices[tet[2]], vertices[tet[3]]);
...
double tet_volume(const geometry::point_t &v0, const geometry::point_t &v1,
  ...
  double cross[3] = {v0v2[1] * v0v3[2] - v0v2[2] * v0v3[1], v0v2[2] * v0v3[0] - v0v2[0] * v0v3[2],
...
double tet_aspect_ratio(const geometry::point_t &v0, const geometry::point_t &v1,
  ...
  auto tri_area = [](const geometry::point_t &a, const geometry::point_t &b,
  ...
  double area =
      tri_area(v0, v1, v2) + tri_area(v0, v1, v3) + tri_area(v0, v2, v3) + tri_area(v1, v2, v3);
...
double tet_min_dihedral_angle(const geometry::point_t &v0, const geometry::point_t &v1,
  ...
  auto face_normal = [](const geometry::point_t &a, const geometry::point_t &b,
    ...
    double ab[3] = {b[0] - a[0], b[1] - a[1], b[2] - a[2]};
    double ac[3] = {c[0] - a[0], c[1] - a[1], c[2] - a[2]};
    std::array<double, 3> n = {{ab[1] * ac[2] - ab[2] * ac[1], ab[2] * ac[0] - ab[0] * ac[2],
                                ab[0] * ac[1] - ab[1] * ac[0]}};
...
double hex_jacobian(const geometry::point_t vertices[8]) {
  ...
  double det = dxdr * (dyds * dzdt - dydt * dzds) - dydr * (dxds * dzdt - dxdt * dzds) +
...
quality_stats compute_tet_quality_stats(const geometry::tets_t &tets,
  ...
  for (const auto &tet : tets) {
    ...
    case TET_VOLUME:
      val = std::abs(
          tet_volume(vertices[tet[0]], vertices[tet[1]], vertices[tet[2]], vertices[tet[3]]));
      break;
    case TET_ASPECT_RATIO:
      val =
          tet_aspect_ratio(vertices[tet[0]], vertices[tet[1]], vertices[tet[2]], vertices[tet[3]]);
      break;
    case TET_MIN_ANGLE:
      val = tet_min_dihedral_angle(vertices[tet[0]], vertices[tet[1]], vertices[tet[2]],
                                   vertices[tet[3]]);
      break;
    default:
      val =
...
std::vector<size_t> find_tets_containing_point(const geometry::point_t &p,
  ...
  if (tets.size() > 10000) {
    ...
    for (const auto &tri : surface.const_tris()) {
      Point_3 p0(surface.points()[tri[0]][0], surface.points()[tri[0]][1],
                 surface.points()[tri[0]][2]);
      Point_3 p1(surface.points()[tri[1]][0], surface.points()[tri[1]][1],
                 surface.points()[tri[1]][2]);
      Point_3 p2(surface.points()[tri[2]][0], surface.points()[tri[2]][1],
                 surface.points()[tri[2]][2]);
    ...
    for (const auto &elem : elements) {
      if (elem.bbox.contains(p[0], p[1], p[2])) {
        ...
        auto weights = tet_barycentric(p, vertices[tet[0]], vertices[tet[1]], vertices[tet[2]],
                                       vertices[tet[3]]);
  ...
  for (size_t i = 0; i < tets.size(); ++i) {
    ...
    if (use_bbox_filter) {
      ...
      if (p[0] < min_x || p[0] > max_x || p[1] < min_y || p[1] > max_y || p[2] < min_z ||
          p[2] > max_z) {
        continue;
      }
    }
...
std::vector<size_t> filter_tets_by_quality(const geometry::tets_t &tets,
  ...
  for (size_t i = 0; i < tets.size(); ++i) {
    ...
    case TET_VOLUME:
      ...
      break;
    case TET_MIN_ANGLE:
      quality = tet_min_dihedral_angle(vertices[tet[0]], vertices[tet[1]], vertices[tet[2]],
                                       vertices[tet[3]]);
...
geometry extract_quality_elements(const geometry &geom, double threshold, quality_metric metric) {
  ...
  if (geom.num_tets() > 0) {
    auto good_tets = filter_tets_by_quality(geom.const_tets(), geom.points(), threshold, metric);
    for (size_t idx : good_tets) {
      result.tets().push_back(geom.const_tets()[idx]);
    }
  } else if (geom.num_hexs() > 0) {
    auto good_hexs = filter_hexs_by_quality(geom.const_hexs(), geom.points(), threshold, metric);
...
geometry generate_sphere(double cx, double cy, double cz, double radius, int thetaRes, int phiRes) {
  geometry geom;
  ...
  for (int j = 0; j < thetaRes; ++j) {
    ...
    geom.tris().push_back(
        tri_t{0, static_cast<unsigned int>(1 + j), static_cast<unsigned int>(1 + next)});
  }
  ...
  for (int i = 0; i < phiRes - 2; ++i) {
    ...
    for (int j = 0; j < thetaRes; ++j) {
      ...
      geom.tris().push_back(tri_t{static_cast<unsigned int>(rowStart + j),
  ...
  for (int j = 0; j < thetaRes; ++j) {
    int next = (j + 1) % thetaRes;
    geom.tris().push_back(tri_t{static_cast<unsigned int>(lastRowStart + j),
...
geometry generate_cube(double cx, double cy, double cz, double sizeX, double sizeY, double sizeZ) {
  ...
  // Front face (+Z)
  geom.points().push_back(vertices[4]);
  geom.normals().push_back({0, 0, 1});
  geom.points().push_back(vertices[5]);
  geom.normals().push_back({0, 0, 1});
  geom.points().push_back(vertices[6]);
  geom.normals().push_back({0, 0, 1});
  geom.points().push_back(vertices[7]);
  geom.normals().push_back({0, 0, 1});
  geom.tris().push_back({0, 1, 2});
  geom.tris().push_back({0, 2, 3});

  // Back face (-Z)
  geom.points().push_back(vertices[1]);
  geom.normals().push_back({0, 0, -1});
  geom.points().push_back(vertices[0]);
  geom.normals().push_back({0, 0, -1});
  geom.points().push_back(vertices[3]);
  geom.normals().push_back({0, 0, -1});
  geom.points().push_back(vertices[2]);
  geom.normals().push_back({0, 0, -1});
...
geometry generate_torus(double cx, double cy, double cz, double majorRadius, double minorRadius,
  ...
  for (int i = 0; i < majorRes; ++i) {
    ...
    for (int j = 0; j < minorRes; ++j) {
      ...
      geom.tris().push_back(tri_t{static_cast<unsigned int>(v0), static_cast<unsigned int>(v1),
                                  static_cast<unsigned int>(v2)});
      geom.tris().push_back(tri_t{static_cast<unsigned int>(v0), static_cast<unsigned int>(v2),
...
geometry generate_cone(double cx, double cy, double cz, double radius, double height, int res) {
  geometry geom;
  ...
  for (int i = 0; i < res; ++i) {
    int next = (i + 1) % res;
    geom.tris().push_back(
        tri_t{0, static_cast<unsigned int>(1 + next), static_cast<unsigned int>(1 + i)});
  }
  ...
  for (int i = 0; i < res; ++i) {
    int next = (i + 1) % res;
    geom.tris().push_back(tri_t{static_cast<unsigned int>(baseCenterIdx),
