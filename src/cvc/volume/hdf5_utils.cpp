...
H5::PredType getPredType(data_type vt) {
  ...
  case Undefined:
    throw hdf5_exception("No HDF5 PredType for undefined type");
  default:
    throw hdf5_exception("No H5::PredType for cvc::data_type " +
...
boost::shared_ptr<H5::H5File> getH5File(const std::string &filename, bool create) {
  using namespace H5;
...
boost::shared_ptr<H5::Group> getGroup(const H5::H5File &file, const std::string &groupPath,
  ...
  while (!groupNames.empty()) {
    ...
    try {
      cvc_group.reset(new Group(cvc_group ? cvc_group->openGroup(name) : file.openGroup(name)));
    } catch (H5::Exception &e) {
      if (create)
        cvc_group.reset(
            new Group(cvc_group ? cvc_group->createGroup(name) : file.createGroup(name)));
      else
...
void unlink(const H5::H5File &file, const std::string &objectPath) {
  ...
  if (cvc_group)
    cvc_group->unlink(unlinkName);
  else
    const_cast<H5::H5File &>(file).unlink(unlinkName);
}
...
bool objectExists(app &ctx, const std::string &hdf5_filename, const std::string &hdf5_objname) {
  ctx.log(10, boost::str(boost::format("%1%: %2%, %3%\n") % BOOST_CURRENT_FUNCTION % hdf5_filename %
                         hdf5_objname));
  return isGroup(ctx, hdf5_filename, hdf5_objname) || isDataSet(ctx, hdf5_filename, hdf5_objname);
}
...
void createHDF5File(app &ctx, const std::string &hdf5_filename) {
  ...
  try {
    ...
    throw hdf5_exception(
        str(boost::format("filename: %s, msg: %s") % hdf5_filename % error.getDetailMsg()));
  }
}
...
//   specified object path if necessary.
// ---- Change History ----
// 07/15/2011 -- Joe R. -- Creation, adapted from VolMagick/HDF5_IO.cpp
...
void createDataSet(app &ctx, const std::string &hdf5_filename, const std::string &hdf5_objname,
                   const bounding_box &boundingBox, const dimension &dim, data_type dataType,
                   const bool replace, const bool createGroups) {
  using namespace H5;
  using namespace boost;
...

  {
    ...
    try {
      ...
      ctx.log(10, str(boost::format("%1%: group: %2%\n") % BOOST_CURRENT_FUNCTION % groupPath));
      shared_ptr<Group> cvc_group = getGroup(*file, groupPath, createGroups);
      ...
      hsize_t chunk_dim[RANK] = {std::min(maxdim[2], dim[2]), std::min(maxdim[1], dim[1]),
                                 std::min(maxdim[0], dim[0])};
      ...
      case UChar: {
        ...
        if (cvc_group)
          dataset.reset(new DataSet(
              cvc_group->createDataSet(dataSetName, PredType::NATIVE_UCHAR, dataspace, plist)));
        else
          dataset.reset(new DataSet(
              file->createDataSet(dataSetName, PredType::NATIVE_UCHAR, dataspace, plist)));
      } break;
      case UShort: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        if (cvc_group)
          dataset.reset(new DataSet(
              cvc_group->createDataSet(dataSetName, PredType::NATIVE_USHORT, dataspace, plist)));
        else
          dataset.reset(new DataSet(
              file->createDataSet(dataSetName, PredType::NATIVE_USHORT, dataspace, plist)));
      } break;
      case UInt: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        if (cvc_group)
          dataset.reset(new DataSet(
              cvc_group->createDataSet(dataSetName, PredType::NATIVE_UINT, dataspace, plist)));
        else
          dataset.reset(new DataSet(
              file->createDataSet(dataSetName, PredType::NATIVE_UINT, dataspace, plist)));
      } break;
      case Float: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        if (cvc_group)
          dataset.reset(new DataSet(
              cvc_group->createDataSet(dataSetName, PredType::NATIVE_FLOAT, dataspace, plist)));
        else
          dataset.reset(new DataSet(
              file->createDataSet(dataSetName, PredType::NATIVE_FLOAT, dataspace, plist)));
      } break;
      case Double: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        if (cvc_group)
          dataset.reset(new DataSet(
              cvc_group->createDataSet(dataSetName, PredType::NATIVE_DOUBLE, dataspace, plist)));
        else
          dataset.reset(new DataSet(
              file->createDataSet(dataSetName, PredType::NATIVE_DOUBLE, dataspace, plist)));
      } break;
      case UInt64: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        if (cvc_group)
          dataset.reset(new DataSet(
              cvc_group->createDataSet(dataSetName, PredType::NATIVE_UINT64, dataspace, plist)));
        else
          dataset.reset(new DataSet(
              file->createDataSet(dataSetName, PredType::NATIVE_UINT64, dataspace, plist)));
      } break;
      case Char: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        if (cvc_group)
          dataset.reset(
              new DataSet(cvc_group->createDataSet(dataSetName, PredType::C_S1, dataspace, plist)));
        else
          dataset.reset(
              new DataSet(file->createDataSet(dataSetName, PredType::C_S1, dataspace, plist)));
      } break;
      default:
        throw write_error(
            str(boost::format("%1% : invalid type :: %2%") % BOOST_CURRENT_FUNCTION % dataType));
        break;
      }
    } catch (H5::Exception &error) {
      using namespace boost;
...
// 08/28/2011 -- Joe R. -- Throwing exception instead of using boolean ret val
void createDataSet(app &ctx, const std::string &hdf5_filename, const std::string &hdf5_objname,
                   const std::string &value, bool createGroups) {
  ctx.log(10, boost::str(boost::format("%1%: %2%, %3%, %4%\n") % BOOST_CURRENT_FUNCTION %
                         hdf5_filename % hdf5_objname % value));
  createDataSet(ctx, hdf5_filename, hdf5_objname, dimension(value.size(), 1, 1), Char,
                createGroups);
  writeDataSet(ctx, hdf5_filename, hdf5_objname, value);
}
...
void readDataSet(app &ctx, const std::string &hdf5_filename, const std::string &hdf5_objname,
  ...
  readDataSet(ctx, hdf5_filename, hdf5_objname, 0, 0, 0, dim, &(value[0]));
}
...
void writeDataSet(app &ctx, const std::string &hdf5_filename, const std::string &hdf5_objname,
  ...
  // const?
  writeDataSet<char>(ctx, hdf5_filename, hdf5_objname, 0, 0, 0, dimension(value.size(), 1, 1),
                     &(value[0]), -128, 127);
}
...
dimension getDataSetDimensionForBoundingBox(app &ctx, const std::string &hdf5_filename,
  ...
  dimension fulldim(1 + (subvolbox.maxx - subvolbox.minx) / xspan,
...
dimension getDataSetDimension(app &ctx, const std::string &hdf5_filename,
  ...
  if (fulldim.isNull()) {
    throw hdf5_exception(str(boost::format("filename: %s, object: %s, msg: %s") % hdf5_filename %
                             hdf5_objname % "Null voxel selection"));
  }

  hsize_t off_x = (subvolbox.minx - boundingBox.minx) / xspan;
  hsize_t off_y = (subvolbox.miny - boundingBox.miny) / yspan;
  hsize_t off_z = (subvolbox.minz - boundingBox.minz) / zspan;
