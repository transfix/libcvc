...
size_t _FREAD(void *dest, size_t size, size_t nitems, FILE *&fp, int reverse) {
  ...
  if (reverse) {
    ...
    case 8:
      ...
      break;
    default:
      break; // ignore weird sizes for now
    }
  }
...
// ---- Change History ----
// ??/??/2008 -- Joe R. -- Creation.
int readSpiderFile(const char *filename, char filetype, int dim[], float **data) {
  FILE *fp = NULL;
  union {
    unsigned char c[4];
    float f;
  ...
  SpiderHeader header;
  unsigned long usfNcol, usfNrow, usfNslice, usfHeader, size, tmpSize, volSize, n, currentPosition;
  struct stat info;
  float *ptr;
  ...
  switch (filetype) {
  case 'V':
    if (file_type.c[0] == 0 && file_type.c[1] == 0 && file_type.c[2] == 64 &&
        file_type.c[3] == 64) {
      fileReversed = 0;
    } else if (file_type.c[0] == 64 && file_type.c[1] == 64 && file_type.c[2] == 0 &&
               file_type.c[3] == 0) {
      fileReversed = 1;
    } else {
      fclose(fp);
    ...
    break;
  case 'I':
    if (file_type.c[0] == 0 && file_type.c[1] == 0 && file_type.c[2] == 128 &&
        file_type.c[3] == 63) {
      fileReversed = 0;
    } else if (file_type.c[0] == 63 && file_type.c[1] == 128 && file_type.c[2] == 0 &&
               file_type.c[3] == 0) {
      fileReversed = 1;
    } else {
      fclose(fp);
  ...
  /* Check machine type */
  file_type.f = 1;
  if (file_type.c[0] == 63 && file_type.c[1] == 128 && file_type.c[2] == 0 && file_type.c[3] == 0)
    machineReversed = 1;
  ...
  usfHeader = (unsigned long)header.fNcol * (unsigned long)header.fLabrec * sizeof(float);
...
// ---- Change History ----
// ??/??/2008 -- Joe R. -- Creation.
int writeSpiderFile(const char *filename, char filetype, int dim[], float *data) {
  FILE *fp;
  SpiderHeader header;
  ...
  header.fSig = -1;
  header.fImami = header.fFmax = header.fFmin = header.fAv = header.fIhist = header.fIangle =
      header.fPhi = header.fTheta = header.fPsi = header.fXoff = header.fYoff = header.fZoff =
          header.fPhi1 = header.fTheta1 = header.fPsi1 = header.fPhi2 = header.fTheta2 =
              header.fPsi2 = header.fAngle1 = header.fr1 = header.fr2 = header.RTflag =
                  header.Astart = header.Aend = header.Ainc = header.Rsigma = header.Tstart =
                      header.Tend = header.Tinc = header.Weight = header.Flip = 0.0F;
  header.szIDat[0] = header.szITim[0] = header.szITit[0] = header.fNada2[0] = header.fNada[0] = 0;
...
struct spider_io : public volume_file_io {
  ...
  virtual void getVolumeFileInfo(app &ctx, volume_file_info::data &d,
    ...
    d._boundingBox =
        bounding_box(0.0, 0.0, 0.0, double(dim[0] - 1), double(dim[1] - 1), double(dim[2] - 1));
  ...
  virtual void readVolumeFile(app &ctx, volume &vol, const std::string &filename, unsigned int var,
    ...
    vol.boundingBox(bounding_box(double(off_x), double(off_y), double(off_z),
                                 double(off_x + subvoldim[0] - 1), double(off_y + subvoldim[1] - 1),
  ...
  virtual void createVolumeFile(app &ctx, const std::string &filename,
    ...
    if (numVariables > 1)
      throw invalid_spider_file(str(
          boost::format("Spider format only supports 1 variable (%d requested)") % numVariables));
    if (numTimesteps > 1)
      throw invalid_spider_file(str(
          boost::format("Spider format only supports 1 timestep (%d requested)") % numTimesteps));
    if (voxelTypes.size() > 1)
      throw invalid_spider_file("Spider format only supports 1 variable and 1 timestep. (too many "
                                "voxel types specified)");
    if (voxelTypes[0] != Float)
      throw invalid_spider_file("Spider format only supports float valued voxels.");
    if (min_time != max_time)
      throw invalid_spider_file("Spider format does not support multiple timesteps. (min time and "
  ...
  virtual void writeVolumeFile(app &ctx, const volume &wvol, const std::string &filename,
    ...
    int dim[] = {static_cast<int>(vol.XDim()), static_cast<int>(vol.YDim()),
                 static_cast<int>(vol.ZDim())};
    vol.voxelType(
        Float); // make sure we have a float volume (though we shouldnt get here if we dont)
