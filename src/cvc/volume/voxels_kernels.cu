...
__device__ inline double getTriVal_device(double val[8], double x, double y, double z, double resX,
...
template <typename T>
__global__ void resize_trilinear_kernel(const T *src_data, T *dst_data, uint64 src_x, uint64 src_y,
...
template __global__ void resize_trilinear_kernel<unsigned char>(const unsigned char *,
                                                                unsigned char *, uint64, uint64,
                                                                uint64, uint64, uint64, uint64,
                                                                double, double, double);

template __global__ void resize_trilinear_kernel<unsigned short>(const unsigned short *,
                                                                 unsigned short *, uint64, uint64,
                                                                 uint64, uint64, uint64, uint64,
                                                                 double, double, double);

template __global__ void resize_trilinear_kernel<unsigned int>(const unsigned int *, unsigned int *,
                                                               uint64, uint64, uint64, uint64,
                                                               uint64, uint64, double, double,
                                                               double);

template __global__ void resize_trilinear_kernel<float>(const float *, float *, uint64, uint64,
                                                        uint64, uint64, uint64, uint64, double,
                                                        double, double);

template __global__ void resize_trilinear_kernel<double>(const double *, double *, uint64, uint64,
                                                         uint64, uint64, uint64, uint64, double,
                                                         double, double);

template __global__ void resize_trilinear_kernel<uint64>(const uint64 *, uint64 *, uint64, uint64,
                                                         uint64, uint64, uint64, uint64, double,
                                                         double, double);
...
extern "C" void cuda_resize_trilinear(void *src_data, void *dst_data, uint64 src_x, uint64 src_y,
  ...
  dim3 gridSize((dst_x + blockSize.x - 1) / blockSize.x, (dst_y + blockSize.y - 1) / blockSize.y,
  ...
  case UChar:
    resize_trilinear_kernel<unsigned char><<<gridSize, blockSize>>>(
        static_cast<const unsigned char *>(src_data), static_cast<unsigned char *>(dst_data), src_x,
        src_y, src_z, dst_x, dst_y, dst_z, inSpaceX, inSpaceY, inSpaceZ);
    break;

  case UShort:
    resize_trilinear_kernel<unsigned short><<<gridSize, blockSize>>>(
        static_cast<const unsigned short *>(src_data), static_cast<unsigned short *>(dst_data),
        src_x, src_y, src_z, dst_x, dst_y, dst_z, inSpaceX, inSpaceY, inSpaceZ);
    break;

  case UInt:
    resize_trilinear_kernel<unsigned int><<<gridSize, blockSize>>>(
        static_cast<const unsigned int *>(src_data), static_cast<unsigned int *>(dst_data), src_x,
        src_y, src_z, dst_x, dst_y, dst_z, inSpaceX, inSpaceY, inSpaceZ);
    break;

  case Float:
    resize_trilinear_kernel<float><<<gridSize, blockSize>>>(
        static_cast<const float *>(src_data), static_cast<float *>(dst_data), src_x, src_y, src_z,
        dst_x, dst_y, dst_z, inSpaceX, inSpaceY, inSpaceZ);
    break;

  case Double:
    resize_trilinear_kernel<double><<<gridSize, blockSize>>>(
        static_cast<const double *>(src_data), static_cast<double *>(dst_data), src_x, src_y, src_z,
        dst_x, dst_y, dst_z, inSpaceX, inSpaceY, inSpaceZ);
    break;

  case UInt64:
    resize_trilinear_kernel<uint64><<<gridSize, blockSize>>>(
        static_cast<const uint64 *>(src_data), static_cast<uint64 *>(dst_data), src_x, src_y, src_z,
        dst_x, dst_y, dst_z, inSpaceX, inSpaceY, inSpaceZ);
    break;
  }
...
template <typename T>
__global__ void resize_bbox_trilinear_kernel(const T *src_data, T *dst_data, uint64 dim_x,
...
extern "C" void cuda_resize_bbox_trilinear(void *src_data, void *dst_data, uint64 dim_x,
  ...
  // Configure kernel launch parameters
  dim3 blockSize(8, 8, 8); // 512 threads per block
  dim3 gridSize((dim_x + blockSize.x - 1) / blockSize.x, (dim_y + blockSize.y - 1) / blockSize.y,
  ...
  case UChar:
    resize_bbox_trilinear_kernel<unsigned char><<<gridSize, blockSize>>>(
        static_cast<const unsigned char *>(src_data), static_cast<unsigned char *>(dst_data), dim_x,
        dim_y, dim_z, offset_x, offset_y, offset_z, scale_x, scale_y, scale_z);
    break;

  case UShort:
    resize_bbox_trilinear_kernel<unsigned short><<<gridSize, blockSize>>>(
        static_cast<const unsigned short *>(src_data), static_cast<unsigned short *>(dst_data),
        dim_x, dim_y, dim_z, offset_x, offset_y, offset_z, scale_x, scale_y, scale_z);
    break;

  case UInt:
    resize_bbox_trilinear_kernel<unsigned int><<<gridSize, blockSize>>>(
        static_cast<const unsigned int *>(src_data), static_cast<unsigned int *>(dst_data), dim_x,
        dim_y, dim_z, offset_x, offset_y, offset_z, scale_x, scale_y, scale_z);
    break;

  case Float:
    resize_bbox_trilinear_kernel<float><<<gridSize, blockSize>>>(
        static_cast<const float *>(src_data), static_cast<float *>(dst_data), dim_x, dim_y, dim_z,
        offset_x, offset_y, offset_z, scale_x, scale_y, scale_z);
    break;

  case Double:
    resize_bbox_trilinear_kernel<double><<<gridSize, blockSize>>>(
        static_cast<const double *>(src_data), static_cast<double *>(dst_data), dim_x, dim_y, dim_z,
        offset_x, offset_y, offset_z, scale_x, scale_y, scale_z);
    break;

  case UInt64:
    resize_bbox_trilinear_kernel<uint64><<<gridSize, blockSize>>>(
        static_cast<const uint64 *>(src_data), static_cast<uint64 *>(dst_data), dim_x, dim_y, dim_z,
        offset_x, offset_y, offset_z, scale_x, scale_y, scale_z);
    break;
  }
...
template <typename T>
__global__ void minmax_reduction_kernel(const T *data, double *block_mins, double *block_maxs,
                                        uint64 off_x, uint64 off_y, uint64 off_z, uint64 dim_x,
                                        uint64 dim_y, uint64 dim_z, uint64 vol_x, uint64 vol_y,
                                        uint64 vol_z) {
...
extern "C" double cuda_compute_min(void *data, uint64 off_x, uint64 off_y, uint64 off_z,
  ...
  case UChar:
    minmax_reduction_kernel<unsigned char><<<gridSize, blockSize, shared_mem_size>>>(
        static_cast<const unsigned char *>(data), d_block_mins, d_block_maxs, off_x, off_y, off_z,
        dim_x, dim_y, dim_z, vol_x, vol_y, vol_z);
    break;
  case UShort:
    minmax_reduction_kernel<unsigned short><<<gridSize, blockSize, shared_mem_size>>>(
        static_cast<const unsigned short *>(data), d_block_mins, d_block_maxs, off_x, off_y, off_z,
        dim_x, dim_y, dim_z, vol_x, vol_y, vol_z);
    break;
  case UInt:
    minmax_reduction_kernel<unsigned int><<<gridSize, blockSize, shared_mem_size>>>(
        static_cast<const unsigned int *>(data), d_block_mins, d_block_maxs, off_x, off_y, off_z,
        dim_x, dim_y, dim_z, vol_x, vol_y, vol_z);
    break;
  case Float:
    minmax_reduction_kernel<float><<<gridSize, blockSize, shared_mem_size>>>(
        static_cast<const float *>(data), d_block_mins, d_block_maxs, off_x, off_y, off_z, dim_x,
        dim_y, dim_z, vol_x, vol_y, vol_z);
    break;
  case Double:
    minmax_reduction_kernel<double><<<gridSize, blockSize, shared_mem_size>>>(
        static_cast<const double *>(data), d_block_mins, d_block_maxs, off_x, off_y, off_z, dim_x,
        dim_y, dim_z, vol_x, vol_y, vol_z);
    break;
  case UInt64:
    minmax_reduction_kernel<uint64><<<gridSize, blockSize, shared_mem_size>>>(
        static_cast<const uint64 *>(data), d_block_mins, d_block_maxs, off_x, off_y, off_z, dim_x,
        dim_y, dim_z, vol_x, vol_y, vol_z);
    break;
  }
  ...
  CUDA_CHECK(cudaMemcpy(h_block_mins.data(), d_block_mins, num_blocks * sizeof(double),
...
extern "C" double cuda_compute_max(void *data, uint64 off_x, uint64 off_y, uint64 off_z,
  ...
  CUDA_CHECK(cudaMemcpy(h_block_maxs.data(), d_block_maxs, num_blocks * sizeof(double),
                        cudaMemcpyDeviceToHost));
...
template <typename T>
__global__ void anisotropic_diffusion_slice_kernel(const T *src_data, T *dst_data, uint64 xdim,
                                                   uint64 ydim, uint64 slice_idx, uint64 zdim,
  ...
  double new_val = current_val + Lambda_para * (cn * delta_n + cs * delta_s + ce * delta_e +
...
// Explicit template instantiations for all voxel types
template __global__ void anisotropic_diffusion_slice_kernel<unsigned char>(const unsigned char *,
                                                                           unsigned char *, uint64,
                                                                           uint64, uint64, uint64,
                                                                           double, double);

template __global__ void anisotropic_diffusion_slice_kernel<unsigned short>(const unsigned short *,
                                                                            unsigned short *,
                                                                            uint64, uint64, uint64,
                                                                            uint64, double, double);

template __global__ void anisotropic_diffusion_slice_kernel<unsigned int>(const unsigned int *,
                                                                          unsigned int *, uint64,
                                                                          uint64, uint64, uint64,
                                                                          double, double);

template __global__ void anisotropic_diffusion_slice_kernel<float>(const float *, float *, uint64,
                                                                   uint64, uint64, uint64, double,
                                                                   double);

template __global__ void anisotropic_diffusion_slice_kernel<double>(const double *, double *,
                                                                    uint64, uint64, uint64, uint64,
                                                                    double, double);

template __global__ void anisotropic_diffusion_slice_kernel<uint64>(const uint64 *, uint64 *,
                                                                    uint64, uint64, uint64, uint64,
                                                                    double, double);
...
extern "C" void cuda_anisotropic_diffusion_slice(void *src_data, void *dst_data, uint64 xdim,
  ...
  dim3 gridSize((xdim + blockSize.x - 1) / blockSize.x, (ydim + blockSize.y - 1) / blockSize.y);

  // Launch kernel based on voxel type
  switch (voxel_type) {
  case UChar:
    anisotropic_diffusion_slice_kernel<unsigned char><<<gridSize, blockSize>>>(
        static_cast<const unsigned char *>(src_data), static_cast<unsigned char *>(dst_data), xdim,
        ydim, slice_idx, zdim, K_para, Lambda_para);
    break;
  ...
  case UInt:
    anisotropic_diffusion_slice_kernel<unsigned int><<<gridSize, blockSize>>>(
        static_cast<const unsigned int *>(src_data), static_cast<unsigned int *>(dst_data), xdim,
        ydim, slice_idx, zdim, K_para, Lambda_para);
    break;

  case Float:
    anisotropic_diffusion_slice_kernel<float><<<gridSize, blockSize>>>(
        static_cast<const float *>(src_data), static_cast<float *>(dst_data), xdim, ydim, slice_idx,
        zdim, K_para, Lambda_para);
    break;

  case Double:
    anisotropic_diffusion_slice_kernel<double><<<gridSize, blockSize>>>(
        static_cast<const double *>(src_data), static_cast<double *>(dst_data), xdim, ydim,
        slice_idx, zdim, K_para, Lambda_para);
    break;

  case UInt64:
    anisotropic_diffusion_slice_kernel<uint64><<<gridSize, blockSize>>>(
        static_cast<const uint64 *>(src_data), static_cast<uint64 *>(dst_data), xdim, ydim,
        slice_idx, zdim, K_para, Lambda_para);
    break;
  }
...
template <typename T>
__global__ void bilateral_filter_kernel(const T *src_data, T *dst_data, uint64 xdim, uint64 ydim,
                                        uint64 zdim, const double *radiometricTable,
                                        const double *spatialMask, int filterRadius,
                                        double valueRange) {
  // 3D grid indexing
  uint64 i = blockIdx.x * blockDim.x + threadIdx.x;
  uint64 j = blockIdx.y * blockDim.y + threadIdx.y;
...
template __global__ void bilateral_filter_kernel<unsigned char>(const unsigned char *,
                                                                unsigned char *, uint64, uint64,
                                                                uint64, const double *,
                                                                const double *, int, double);

template __global__ void bilateral_filter_kernel<unsigned short>(const unsigned short *,
                                                                 unsigned short *, uint64, uint64,
                                                                 uint64, const double *,
                                                                 const double *, int, double);

template __global__ void bilateral_filter_kernel<unsigned int>(const unsigned int *, unsigned int *,
                                                               uint64, uint64, uint64,
                                                               const double *, const double *, int,
                                                               double);

template __global__ void bilateral_filter_kernel<float>(const float *, float *, uint64, uint64,
                                                        uint64, const double *, const double *, int,
                                                        double);

template __global__ void bilateral_filter_kernel<double>(const double *, double *, uint64, uint64,
                                                         uint64, const double *, const double *,
                                                         int, double);

template __global__ void bilateral_filter_kernel<uint64>(const uint64 *, uint64 *, uint64, uint64,
                                                         uint64, const double *, const double *,
                                                         int, double);
...
extern "C" void cuda_bilateral_filter(void *src_data, void *dst_data, uint64 xdim, uint64 ydim,
  ...
  CUDA_CHECK(cudaMemcpy(d_radiometricTable, h_radiometricTable, 256 * sizeof(double),
  ...
  for (int k = -int(filterRadius); k <= int(filterRadius); k++) {
    for (int j = -int(filterRadius); j <= int(filterRadius); j++) {
      for (int i = -int(filterRadius); i <= int(filterRadius); i++) {
        h_spatialMask[index++] =
            exp(double(k * k + j * j + i * i) / (-spatialSigma * spatialSigma * 2.0));
      }
    }
  }
  CUDA_CHECK(
      cudaMemcpy(d_spatialMask, h_spatialMask, maskSize * sizeof(double), cudaMemcpyHostToDevice));
  ...
  case UChar:
    bilateral_filter_kernel<unsigned char><<<gridSize, blockSize>>>(
        static_cast<const unsigned char *>(src_data), static_cast<unsigned char *>(dst_data), xdim,
        ydim, zdim, d_radiometricTable, d_spatialMask, filterRadius, valueRange);
    break;

  case UShort:
    bilateral_filter_kernel<unsigned short><<<gridSize, blockSize>>>(
        static_cast<const unsigned short *>(src_data), static_cast<unsigned short *>(dst_data),
        xdim, ydim, zdim, d_radiometricTable, d_spatialMask, filterRadius, valueRange);
    break;
  ...
  case Float:
    bilateral_filter_kernel<float><<<gridSize, blockSize>>>(
        static_cast<const float *>(src_data), static_cast<float *>(dst_data), xdim, ydim, zdim,
        d_radiometricTable, d_spatialMask, filterRadius, valueRange);
    break;

  case Double:
    bilateral_filter_kernel<double><<<gridSize, blockSize>>>(
        static_cast<const double *>(src_data), static_cast<double *>(dst_data), xdim, ydim, zdim,
        d_radiometricTable, d_spatialMask, filterRadius, valueRange);
    break;

  case UInt64:
    bilateral_filter_kernel<uint64><<<gridSize, blockSize>>>(
        static_cast<const uint64 *>(src_data), static_cast<uint64 *>(dst_data), xdim, ydim, zdim,
        d_radiometricTable, d_spatialMask, filterRadius, valueRange);
    break;
  }
...
template <typename T>
__global__ void
contrast_enhancement_stretching_kernel(const T *src_data,     // Original image data
...
template __global__ void contrast_enhancement_stretching_kernel<unsigned char>(
    ...
    const unsigned char *, const unsigned char *, unsigned char *, uint64, uint64, uint64, double,
    double);
...
template __global__ void contrast_enhancement_stretching_kernel<unsigned short>(
    ...
    const unsigned short *, const unsigned short *, unsigned short *, uint64, uint64, uint64,
    double, double);
...
template __global__ void contrast_enhancement_stretching_kernel<unsigned int>(
    ...
    const unsigned int *, const unsigned int *, unsigned int *, uint64, uint64, uint64, double,
    double);

template __global__ void contrast_enhancement_stretching_kernel<float>(const float *, const float *,
...
contrast_enhancement_stretching_kernel<double>(const double *, const double *, const double *,
                                               const double *, const double *, const double *,
                                               double *, uint64, uint64, uint64, double, double);

template __global__ void
contrast_enhancement_stretching_kernel<uint64>(const uint64 *, const uint64 *, const uint64 *,
...
extern "C" void cuda_contrast_enhancement_stretching(void *src_data, void *upmin_data,
  ...
  case UChar:
    contrast_enhancement_stretching_kernel<unsigned char><<<gridSize, blockSize>>>(
        ...
        static_cast<const unsigned char *>(imgavg_data), static_cast<unsigned char *>(result_data),
  ...
  case UShort:
    contrast_enhancement_stretching_kernel<unsigned short><<<gridSize, blockSize>>>(
        ...
        static_cast<unsigned short *>(result_data), xdim, ydim, zdim, lmin_global, lmax_global);
    break;

  case UInt:
    contrast_enhancement_stretching_kernel<unsigned int><<<gridSize, blockSize>>>(
        static_cast<const unsigned int *>(src_data), static_cast<const unsigned int *>(upmin_data),
        ...
        static_cast<const unsigned int *>(imgavg_data), static_cast<unsigned int *>(result_data),
        xdim, ydim, zdim, lmin_global, lmax_global);
    break;

  case Float:
    contrast_enhancement_stretching_kernel<float><<<gridSize, blockSize>>>(
        static_cast<const float *>(src_data), static_cast<const float *>(upmin_data),
        static_cast<const float *>(upmax_data), static_cast<const float *>(downmin_data),
        static_cast<const float *>(downmax_data), static_cast<const float *>(imgavg_data),
        static_cast<float *>(result_data), xdim, ydim, zdim, lmin_global, lmax_global);
    break;

  case Double:
    contrast_enhancement_stretching_kernel<double><<<gridSize, blockSize>>>(
        static_cast<const double *>(src_data), static_cast<const double *>(upmin_data),
        static_cast<const double *>(upmax_data), static_cast<const double *>(downmin_data),
        static_cast<const double *>(downmax_data), static_cast<const double *>(imgavg_data),
        static_cast<double *>(result_data), xdim, ydim, zdim, lmin_global, lmax_global);
    break;

  case UInt64:
    contrast_enhancement_stretching_kernel<uint64><<<gridSize, blockSize>>>(
        static_cast<const uint64 *>(src_data), static_cast<const uint64 *>(upmin_data),
        static_cast<const uint64 *>(upmax_data), static_cast<const uint64 *>(downmin_data),
        static_cast<const uint64 *>(downmax_data), static_cast<const uint64 *>(imgavg_data),
        static_cast<uint64 *>(result_data), xdim, ydim, zdim, lmin_global, lmax_global);
    break;
  }
...
__device__ inline float phi_device(float x, float q) { return (2.0f - q) * powf(x, 1.0f - q); }

// Gradient computation kernel (6-neighborhood)
template <typename T>
__global__ void gdtv_gradient_kernel(const T *input_data, float *grad_data, uint64 xdim,
                                     uint64 ydim, uint64 zdim) {
  uint64 i = blockIdx.x * blockDim.x + threadIdx.x;
  uint64 j = blockIdx.y * blockDim.y + threadIdx.y;
  uint64 k = blockIdx.z * blockDim.z + threadIdx.z;
  ...
  if (i + 1 < xdim)
    val[0] = float(input_data[idx + 1]) - center;
  if (i >= 1)
    val[1] = float(input_data[idx - 1]) - center;
  if (j + 1 < ydim)
    val[2] = float(input_data[idx + xdim]) - center;
  if (j >= 1)
    val[3] = float(input_data[idx - xdim]) - center;
  if (k + 1 < zdim)
    val[4] = float(input_data[idx + xdim * ydim]) - center;
  if (k >= 1)
    val[5] = float(input_data[idx - xdim * ydim]) - center;
...
template <typename T>
__global__ void gdtv_filter_kernel(const T *input_data, const float *grad_data,
                                   const T *funcval_data, T *funcvalue_data, uint64 xdim,
                                   uint64 ydim, uint64 zdim, float q, float lbda) {
  uint64 i = blockIdx.x * blockDim.x + threadIdx.x;
  uint64 j = blockIdx.y * blockDim.y + threadIdx.y;
  uint64 k = blockIdx.z * blockDim.z + threadIdx.z;
  ...
  double result = 0.0;
  if (i < xdim - 1)
    result += double(funcval_data[idx + 1]) * temp[0];
  if (i > 1)
    result += double(funcval_data[idx - 1]) * temp[1];
  if (j < ydim - 1)
    result += double(funcval_data[idx + xdim]) * temp[2];
  if (j > 1)
    result += double(funcval_data[idx - xdim]) * temp[3];
  if (k < zdim - 1)
    result += double(funcval_data[idx + xdim * ydim]) * temp[4];
  if (k > 1)
    result += double(funcval_data[idx - xdim * ydim]) * temp[5];
...
template __global__ void gdtv_gradient_kernel<unsigned char>(const unsigned char *, float *, uint64,
                                                             uint64, uint64);
template __global__ void gdtv_gradient_kernel<unsigned short>(const unsigned short *, float *,
                                                              uint64, uint64, uint64);
template __global__ void gdtv_gradient_kernel<unsigned int>(const unsigned int *, float *, uint64,
                                                            uint64, uint64);
template __global__ void gdtv_gradient_kernel<float>(const float *, float *, uint64, uint64,
                                                     uint64);
template __global__ void gdtv_gradient_kernel<double>(const double *, float *, uint64, uint64,
                                                      uint64);
template __global__ void gdtv_gradient_kernel<uint64>(const uint64 *, float *, uint64, uint64,
                                                      uint64);

// Template instantiations for filter kernel
template __global__ void gdtv_filter_kernel<unsigned char>(const unsigned char *, const float *,
                                                           const unsigned char *, unsigned char *,
                                                           uint64, uint64, uint64, float, float);
template __global__ void gdtv_filter_kernel<unsigned short>(const unsigned short *, const float *,
                                                            const unsigned short *,
                                                            unsigned short *, uint64, uint64,
                                                            uint64, float, float);
template __global__ void gdtv_filter_kernel<unsigned int>(const unsigned int *, const float *,
                                                          const unsigned int *, unsigned int *,
                                                          uint64, uint64, uint64, float, float);
template __global__ void gdtv_filter_kernel<float>(const float *, const float *, const float *,
                                                   float *, uint64, uint64, uint64, float, float);
template __global__ void gdtv_filter_kernel<double>(const double *, const float *, const double *,
                                                    double *, uint64, uint64, uint64, float, float);
template __global__ void gdtv_filter_kernel<uint64>(const uint64 *, const float *, const uint64 *,
                                                    uint64 *, uint64, uint64, uint64, float, float);
...
extern "C" void cuda_gdtv_gradient(void *input_data, void *grad_data, uint64 xdim, uint64 ydim,
  ...
  switch (voxel_type) {
  case UChar:
    gdtv_gradient_kernel<unsigned char>
        <<<gridSize, blockSize>>>(static_cast<const unsigned char *>(input_data),
                                  static_cast<float *>(grad_data), xdim, ydim, zdim);
    break;
  case UShort:
    gdtv_gradient_kernel<unsigned short>
        <<<gridSize, blockSize>>>(static_cast<const unsigned short *>(input_data),
                                  static_cast<float *>(grad_data), xdim, ydim, zdim);
    break;
  case UInt:
    gdtv_gradient_kernel<unsigned int>
        <<<gridSize, blockSize>>>(static_cast<const unsigned int *>(input_data),
                                  static_cast<float *>(grad_data), xdim, ydim, zdim);
    break;
  case Float:
    gdtv_gradient_kernel<float><<<gridSize, blockSize>>>(
        static_cast<const float *>(input_data), static_cast<float *>(grad_data), xdim, ydim, zdim);
    break;
  case Double:
    gdtv_gradient_kernel<double><<<gridSize, blockSize>>>(
        static_cast<const double *>(input_data), static_cast<float *>(grad_data), xdim, ydim, zdim);
    break;
  case UInt64:
    gdtv_gradient_kernel<uint64><<<gridSize, blockSize>>>(
        static_cast<const uint64 *>(input_data), static_cast<float *>(grad_data), xdim, ydim, zdim);
    break;
  }
...
extern "C" void cuda_gdtv_filter(void *input_data, void *grad_data, void *funcval_data,
  ...
  case UChar:
    gdtv_filter_kernel<unsigned char><<<gridSize, blockSize>>>(
        static_cast<const unsigned char *>(input_data), static_cast<const float *>(grad_data),
        static_cast<const unsigned char *>(funcval_data),
        static_cast<unsigned char *>(funcvalue_data), xdim, ydim, zdim, q, lbda);
    break;
  case UShort:
    gdtv_filter_kernel<unsigned short><<<gridSize, blockSize>>>(
        static_cast<const unsigned short *>(input_data), static_cast<const float *>(grad_data),
        static_cast<const unsigned short *>(funcval_data),
        static_cast<unsigned short *>(funcvalue_data), xdim, ydim, zdim, q, lbda);
    break;
  case UInt:
    gdtv_filter_kernel<unsigned int><<<gridSize, blockSize>>>(
        static_cast<const unsigned int *>(input_data), static_cast<const float *>(grad_data),
        static_cast<const unsigned int *>(funcval_data),
        static_cast<unsigned int *>(funcvalue_data), xdim, ydim, zdim, q, lbda);
    break;
  case Float:
    gdtv_filter_kernel<float><<<gridSize, blockSize>>>(
        static_cast<const float *>(input_data), static_cast<const float *>(grad_data),
        static_cast<const float *>(funcval_data), static_cast<float *>(funcvalue_data), xdim, ydim,
        zdim, q, lbda);
    break;
  case Double:
    gdtv_filter_kernel<double><<<gridSize, blockSize>>>(
        static_cast<const double *>(input_data), static_cast<const float *>(grad_data),
        static_cast<const double *>(funcval_data), static_cast<double *>(funcvalue_data), xdim,
        ydim, zdim, q, lbda);
    break;
  case UInt64:
    gdtv_filter_kernel<uint64><<<gridSize, blockSize>>>(
        static_cast<const uint64 *>(input_data), static_cast<const float *>(grad_data),
        static_cast<const uint64 *>(funcval_data), static_cast<uint64 *>(funcvalue_data), xdim,
        ydim, zdim, q, lbda);
    break;
  }
