...
struct rawv_io : public volume_file_io {
  ...
  // ---- Change History ----
  // 11/20/2009 -- Joe R. -- Creation.
  rawv_io() : _id("rawv_io : v1.0") { _extensions.push_back(".rawv"); }
  ...
  virtual void getVolumeFileInfo(app &ctx, volume_file_info::data &data,
    ...
    if (sizeof(header_t) + sizeof(var_record_t) * uint64(header.numVariables) >=
        uint64(s.st_size)) {
      fclose(input);
    ...
    for (i = 0; i < header.numVariables; i++) {
      ...
      dataBytes += header.dim[0] * header.dim[1] * header.dim[2] *
                   rawv_type_sizes[var_records[i].varType] * header.numTimesteps;
    }

    if (sizeof(header_t) + sizeof(var_record_t) * uint64(header.numVariables) + dataBytes !=
        uint64(s.st_size)) {
      // arand: added type-cast to eliminate warnings
      SNPRINTF(buf, 255, "File size does not match header info: %llu %llu",
               (long long unsigned int)dataBytes, (long long unsigned int)uint64(s.st_size));
    ...
    */
    ...
    data._boundingBox = bounding_box(header.min[0], header.min[1], header.min[2], header.max[0],
                                     header.max[1], header.max[2]);
  ...
  virtual void readVolumeFile(app &ctx, volume &vol, const std::string &filename, unsigned int var,
    ...
    if ((uint64(sizeof(header_t)) + uint64(sizeof(var_record_t)) * header.numVariables +
         uint64(header.dim[0] * header.dim[1] * header.dim[2]) * rawv_type_sizes[5] *
             header.numTimesteps * header.numVariables) < uint64(s.st_size)) {
      fclose(input);
      throw invalid_rawv_header(
          "File size is greater than the largest possible volume according to the header");
    }
    // make sure that the file size is at least as big as the smallest possible
    //  volume according to the header
    if ((uint64(sizeof(header_t)) + uint64(sizeof(var_record_t)) * header.numVariables +
      ...
      fclose(input);
      throw invalid_rawv_header(
          "File size is smaller than the smallest possible volume according to the header");
    }
    ...
    if (sizeof(header_t) + sizeof(var_record_t) * header.numVariables + dataBytes !=
        uint64(s.st_size)) {
      // arand: type-cast to eliminate warnings
      SNPRINTF(buf, 255, "File size does not match header info: %lld %lld",
               (long long unsigned int)sizeof(header_t) +
                   sizeof(var_record_t) * header.numVariables + dataBytes,
    ...
    if (var >= header.numVariables || time >= header.numTimesteps) {
      ...
      throw index_out_of_bounds(
          "Requested variable/timestep is larger than the number of variables/timesteps");
    }

    /*
       At this point we can be sure that this RawV file and function arguments are correct, so we
       may now modify 'vol'.
    */
    try {
      ...
      subvolbox.setMin(
          header.min[0] + ((header.max[0] - header.min[0]) / (header.dim[0] - 1)) * off_x,
          header.min[1] + ((header.max[1] - header.min[1]) / (header.dim[1] - 1)) * off_y,
          header.min[2] + ((header.max[2] - header.min[2]) / (header.dim[2] - 1)) * off_z);
      subvolbox.setMax(header.min[0] + ((header.max[0] - header.min[0]) / (header.dim[0] - 1)) *
                                           (off_x + subvoldim[0] - 1),
                       header.min[1] + ((header.max[1] - header.min[1]) / (header.dim[1] - 1)) *
                                           (off_y + subvoldim[1] - 1),
                       header.min[2] + ((header.max[2] - header.min[2]) / (header.dim[2] - 1)) *
                                           (off_z + subvoldim[2] - 1));
    ...
    */
    try {
      ...
      for (k = off_z; k <= (off_z + subvoldim[2] - 1); k++) {
        file_offz =
            offset + k * header.dim[0] * header.dim[1] * rawv_type_sizes[var_records[var].varType];
  ...
  virtual void createVolumeFile(app &ctx, const std::string &filename,
    ...
    for (v = 0; v < numVariables; v++) {
      ...
      unsigned char *scanline =
          (unsigned char *)calloc(dimension[0] * data_type_sizes[voxelTypes[v]], 1);
      if (scanline == NULL) {
        fclose(output);
      ...
      for (t = 0; t < numTimesteps; t++)
        for (k = 0; k < dimension[2]; k++)
          for (j = 0; j < dimension[1]; j++) {
            if (fwrite(scanline, data_type_sizes[voxelTypes[v]], dimension[0], output) !=
                dimension[0]) {
              geterrstr(errno, buf, 256);
  ...
  virtual void writeVolumeFile(app &ctx, const volume &wvol, const std::string &filename,
    ...
    try {
      ...
      vol.voxelType(
          volinfo.voxelTypes(var)); // change the volume's voxel type to match that of the file
    } catch (read_error e) {
    ...
    */
    ...
    // set offset to the requested variable
    for (v = 0; v < var; v++)
      offset += volinfo.XDim() * volinfo.YDim() * volinfo.ZDim() * volinfo.voxelSizes(v) *
                volinfo.numTimesteps();

    // set offset to the requested timestep
    offset += volinfo.XDim() * volinfo.YDim() * volinfo.ZDim() * volinfo.voxelSizes(var) * time;
