  ...
  for (uint64 k = 0; k < vol.ZDim(); k++)
    for (uint64 j = 0; j < vol.YDim(); j++)
      for (uint64 i = 0; i < vol.XDim(); i++) {
        double v = vol(i, j, k);
        double normalized =
            (range > 0.0) ? new_min + (v - cur_min) * (new_max - new_min) / range : new_min;
...
volume vol_downsample(const volume &vol, unsigned int fx, unsigned int fy, unsigned int fz) {
  if (fx == 0 || fy == 0 || fz == 0)
  ...
  volume out(vol.ctx(), dimension(nx, ny, nz), Float,
             bounding_box(vol.XMin(), vol.YMin(), vol.ZMin(), new_xmax, new_ymax, new_zmax));
  ...
  for (uint64 i = 0; i < vol.XDim(); i++)
    for (uint64 j = 0; j < vol.YDim(); j++)
      for (uint64 k = 0; k < vol.ZDim(); k++) {
        ...
        if (sx >= vol.XMin() && sx <= vol.XMax() && sy >= vol.YMin() && sy <= vol.YMax())
...
ssim_result vol_ssim(const volume &a, const volume &b, int window_size, double sigma) {
  check_dims_match(a, b);
        ...
        for (int gi = -half; gi <= half; gi++)
          for (int gj = -half; gj <= half; gj++)
            for (int gk = -half; gk <= half; gk++) {
              ...
              if (x < 0 || x >= (int64)a.XDim() || y < 0 || y >= (int64)a.YDim() || z < 0 ||
                  z >= (int64)a.ZDim())
                continue;
              int kidx =
                  (gi + half) * window_size * window_size + (gj + half) * window_size + (gk + half);
...
volume vol_project(const volume &vol, const std::vector<double> &angles_rad, double step) {
  unsigned int na = static_cast<unsigned int>(angles_rad.size());
  if (na == 0)
    throw std::invalid_argument("angles_rad must not be empty");
  ...
  volume out(vol.ctx(), dimension(vol.XDim(), vol.YDim(), na), Float,
             bounding_box(vol.XMin(), vol.YMin(), 0, vol.XMax(), vol.YMax(), (double)na));
    ...
    for (uint64 xi = 0; xi < vol.XDim(); xi++)
      for (uint64 yi = 0; yi < vol.YDim(); yi++) {
        ...
        for (double s = -max_s; s <= max_s; s += step) {
          ...
          if (vx < vol.XMin() || vx > vol.XMax() || vz < vol.ZMin() || vz > vol.ZMax())
            continue;
...
volume vol_back_project(const volume &projections, const std::vector<double> &angles_rad,
                        unsigned int output_dim, bool apply_filter) {
  unsigned int na = static_cast<unsigned int>(angles_rad.size());
  if (na == 0)
    throw std::invalid_argument("angles_rad must not be empty");
    ...
    for (unsigned int y = 0; y < d; y++)
      for (unsigned int z = 0; z < d; z++) {
        ...
        for (unsigned int ai = 0; ai < na; ai++) {
          double theta = angles_rad[ai];
          double xcoord =
              x_shift * std::cos(theta) + z_shift * std::sin(theta) + projections.XDim() / 2.0;

          if (xcoord >= 0 && xcoord < projections.XDim() - 1 && y < projections.YDim()) {
            ...
            double val = (1.0 - frac) * filtered(xi0, y, ai) + frac * filtered(xi1, y, ai);
            accum += val;
            count++;
          }
        }
...
void vol_to_slices(const volume &vol, const std::string &directory, const std::string &format) {
#ifdef CVC_ENABLE_IMAGEMAGICK
  ...
  for (uint64 k = 0; k < norm.ZDim(); k++) {
    ...
    for (uint64 j = 0; j < norm.YDim(); j++)
      for (uint64 i = 0; i < norm.XDim(); i++) {
        unsigned char v = static_cast<unsigned char>(std::min(255.0, std::max(0.0, norm(i, j, k))));
        size_t idx = (j * norm.XDim() + i) * 3;
        buf[idx] = buf[idx + 1] = buf[idx + 2] = v; // grayscale → RGB
      }

    Magick::Image img(norm.XDim(), norm.YDim(), "RGB", Magick::CharPixel, buf.data());
...
volume slices_to_volume(app &ctx, const std::vector<std::string> &paths, const bounding_box &bbox) {
#ifdef CVC_ENABLE_IMAGEMAGICK
  if (paths.empty())
...
volume vol_rgba_merge(const volume &r, const volume &g, const volume &b, const volume &a) {
