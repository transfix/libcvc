...
inline std::uint32_t rotr32(std::uint32_t x, std::uint32_t n) { return (x >> n) | (x << (32 - n)); }
...
std::string sha256_hex_impl(const unsigned char *data, std::size_t len) {
  ...
  for (int w = 0; w < 8; ++w) {
    ...
    for (int b = 0; b < 4; ++b) {
      unsigned char byte = static_cast<unsigned char>((v >> (24 - b * 8)) & 0xffu);
...
state_blob_ref memory_state_blob_store::put(const std::vector<unsigned char> &bytes,
