...
volume_file_io::splitRawFilename(const std::string &filename) {
  ...
  if (names[1].empty())
    names[1] = std::string(CVC_VOLUME_GROUP) + "/" + std::string(DEFAULT_VOLUME_NAME);
...
void volume_file_io::writeBoundingBox(app &ctx, const bounding_box &bbox,
...
void volume_file_io::insertHandler(const ptr &vfio) { insertHandler(handlerMap(), vfio); }
...
void volume_file_io::removeHandler(const ptr &vfio) {
  for (handler_map::iterator i = handlerMap().begin(); i != handlerMap().end(); i++) {
    handlers h;
...
// ---- Change History ----
// 11/20/2009 -- Joe R. -- Creation.
void volume_file_io::insertHandler(handler_map &hm, const ptr &vfio) {
  if (!vfio)
    return;
  for (volume_file_io::extension_list::const_iterator i = vfio->extensions().begin();
...
void readVolumeFile(app &ctx, volume &vol, const std::string &filename, unsigned int var,
  ...
  if (boost::regex_match(actualFileName, what, file_extension)) {
    ...
    for (volume_file_io::handlers::iterator i = h.begin(); i != h.end(); i++)
      try {
        if (*i) {
          (*i)->readVolumeFile(ctx, vol, filename, var, time, off_x, off_y, off_z, subvoldim);
          return;
        }
      } catch (exception &e) {
...
void readVolumeFile(app &ctx, volume &vol, const std::string &filename, unsigned int var,
  ...
  if (boost::regex_match(actualFileName, what, file_extension)) {
    ...
    for (volume_file_io::handlers::iterator i = h.begin(); i != h.end(); i++)
      try {
        if (*i) {
          (*i)->readVolumeFile(ctx, vol, filename, var, time, subvolbox);
          return;
        }
      } catch (exception &e) {
        errors += std::string(" :::: ") + e.what();
...
// ---------------
void writeVolumeFile(app &ctx, const volume &vol, const std::string &filename, unsigned int var,
                     unsigned int time, uint64 off_x, uint64 off_y, uint64 off_z) {
  std::string errors;
  boost::smatch what;
  ...
  if (boost::regex_match(actualFileName, what, file_extension)) {
    ...
    for (volume_file_io::handlers::iterator i = h.begin(); i != h.end(); i++)
      try {
        if (*i) {
          (*i)->writeVolumeFile(ctx, vol, filename, var, time, off_x, off_y, off_z);
          return;
        }
      } catch (exception &e) {
        errors += std::string(" :: ") + e.what();
...
void writeVolumeFile(app &ctx, const volume &vol, const std::string &filename, unsigned int var,
  ...
  localvol.boundingBox(bounding_box(0.0, 0.0, 0.0, 1.0, 1.0, 1.0));
  ...
  for (uint64 k = 0; k < subvol.ZDim(); k++) {
    for (uint64 j = 0; j < subvol.YDim(); j++)
      for (uint64 i = 0; i < subvol.XDim(); i++) {
        subvol(i, j, k, localvol.interpolate(double(i) / xmax, double(j) / ymax, double(k) / zmax));
      }
...
void writeBoundingBox(app &ctx, const bounding_box &bbox, const std::string &filename) {
  ...
  throw unsupported_volume_file_type(boost::str(boost::format("%1% : Cannot write '%2%'%3%") %
                                                BOOST_CURRENT_FUNCTION % filename % errors));
}
...
void readVolumeFile(app &ctx, volume &vol, const std::string &filename, unsigned int var,
                    unsigned int time) {
  volume_file_info volinfo(ctx, filename);
  readVolumeFile(ctx, vol, filename, var, time, 0, 0, 0, volinfo.voxel_dimensions());
}

void readVolumeFile(app &ctx, std::vector<volume> &vols, const std::string &filename) {
  volume_file_info volinfo(ctx, filename);
  volume vol(ctx);
  vols.clear();
...
void writeVolumeFile(app &ctx, const std::vector<volume> &vols, const std::string &filename) {
  uint64 i;
  ...
  createVolumeFile(ctx, filename, vols[0].boundingBox(), vols[0].voxel_dimensions(), voxelTypes,
                   vols.size());
  for (i = 0; i < vols.size(); i++)
    writeVolumeFile(ctx, vols[i], filename, i);
}

void createVolumeFile(app &ctx, const std::string &filename, const bounding_box &boundingBox,
                      const dimension &dimension, const std::vector<data_type> &voxelTypes,
                      unsigned int numVariables, unsigned int numTimesteps, double min_time,
                      double max_time) {
  std::string errors;
  ...
  if (boost::regex_match(actualFileName, what, file_extension)) {
    ...
    for (volume_file_io::handlers::iterator i = h.begin(); i != h.end(); i++)
      try {
        if (*i) {
          (*i)->createVolumeFile(ctx, filename, boundingBox, dimension, voxelTypes, numVariables,
                                 numTimesteps, min_time, max_time);
          return;
        }
      } catch (exception &e) {
