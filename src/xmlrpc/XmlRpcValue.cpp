...
bool XmlRpcValue::operator==(XmlRpcValue const &other) const {
  ...
  switch (_type) {
  case TypeBoolean:
    return (!_value.asBool && !other._value.asBool) || (_value.asBool && other._value.asBool);
  case TypeInt:
...
bool XmlRpcValue::operator!=(XmlRpcValue const &other) const { return !(*this == other); }
...
bool XmlRpcValue::stringFromXml(std::string const &valueXml, int *offset) {
  ...
  _type = TypeString;
  _value.asString =
      new std::string(XmlRpcUtil::xmlDecode(valueXml.substr(*offset, valueEnd - *offset)));
...
std::string XmlRpcValue::timeToXml() const {
  ...
  char buf[20];
  snprintf(buf, sizeof(buf) - 1, "%4d%02d%02dT%02d:%02d:%02d", t->tm_year, t->tm_mon, t->tm_mday,
