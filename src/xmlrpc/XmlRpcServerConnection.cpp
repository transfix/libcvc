...
XmlRpcServerConnection::XmlRpcServerConnection(int fd, XmlRpcServer *server,
...
unsigned XmlRpcServerConnection::handleEvent(unsigned /*eventType*/) {
  ...
  return (_connectionState == WRITE_RESPONSE) ? XmlRpcDispatch::WritableEvent
...
bool XmlRpcServerConnection::executeMethod(const std::string &methodName, XmlRpcValue &params,
                                           XmlRpcValue &result) {
...
bool XmlRpcServerConnection::executeMulticall(const std::string &methodName, XmlRpcValue &params,
                                              XmlRpcValue &result) {
  ...
  for (int i = 0; i < nc; ++i) {
    ...
    if (!params[0][i].hasMember(METHODNAME) || !params[0][i].hasMember(PARAMS)) {
      result[i][FAULTCODE] = -1;
      result[i][FAULTSTRING] =
          SYSTEM_MULTICALL +
