...
bool XmlRpcClient::execute(const char *method, XmlRpcValue const &params, XmlRpcValue &result) {
  XmlRpcUtil::log(1, "XmlRpcClient::execute: method %s (_connectionState %d).", method,
                  _connectionState);
...
unsigned XmlRpcClient::handleEvent(unsigned eventType) {
  if (eventType == XmlRpcDispatch::Exception) {
    if (_connectionState == WRITE_REQUEST && _bytesWritten == 0)
      ...
      XmlRpcUtil::error("Error in XmlRpcClient::handleEvent (state %d): %s.", _connectionState,
                        XmlRpcSocket::getErrorMsg().c_str());
    return 0;
  }
  ...
  return (_connectionState == WRITE_REQUEST) ? XmlRpcDispatch::WritableEvent
...
bool XmlRpcClient::doConnect() {
  ...
  if (!XmlRpcSocket::setNonBlocking(fd)) {
    this->close();
    XmlRpcUtil::error(
        "Error in XmlRpcClient::doConnect: Could not set socket to non-blocking IO mode (%s).",
        XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
...
bool XmlRpcClient::readHeader() {
  // Read available data
  if (!XmlRpcSocket::nbRead(this->getfd(), _header, &_eof) || (_eof && _header.length() == 0)) {
...
bool XmlRpcClient::parseResponse(XmlRpcValue &result) {
  ...
  if ((XmlRpcUtil::nextTagIs(PARAMS_TAG, _response, &offset) &&
    ...
    XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid response - no param or fault "
                      "tag. Response:\n%s",
                      _response.c_str());
    _response = "";
    return false;
