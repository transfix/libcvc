...
bool XmlRpcSocket::listen(int fd, int backlog) { return (::listen(fd, backlog) == 0); }
...
bool XmlRpcSocket::connect(int fd, std::string &host, int port) {
  struct sockaddr_in saddr;
  memset(&saddr, 0, sizeof(saddr));
  saddr.sin_family = AF_INET;
...
bool XmlRpcSocket::nbWrite(int fd, std::string &s, int *bytesSoFar) {
...
std::string XmlRpcSocket::getErrorMsg() { return getErrorMsg(getError()); }
