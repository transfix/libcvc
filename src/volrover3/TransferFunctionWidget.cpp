...
void TransferFunctionWidget::loadTransferFunctionFromVolume(std::shared_ptr<VolumeNode> volume)
{
    if (!volume || !volume->hasVolume()) {
        volrover3::app().log(0, "TransferFunctionWidget::loadTransferFunctionFromVolume: No volume or volume not loaded");
        return;
    }
    ...
    if (minVal.has_value() && maxVal.has_value()) {
        try {
            ...
            volrover3::app().log(0, "  Set data range to: [" + std::to_string(m_dataMin) + ", " + std::to_string(m_dataMax) + "]\n");
        } catch (const std::exception& e) {
            volrover3::app().log(0, "TransferFunctionWidget::loadTransferFunctionFromVolume[" + volume->getName() + 
    ...
    volrover3::app().log(0, "  Raw from state: " + std::to_string(colorTable.size()) + " color values, " +
    ...
    if (!colorTable.empty() && !opacityTable.empty()) {
        ...
        volrover3::app().log(0, "  Loaded TF: " + std::to_string(m_colorPoints.size()) + " color pts, " +
...
std::vector<double> TransferFunctionWidget::getColorTable() const
{
    ...
    volrover3::app().log(0, "  Returning color table with " + std::to_string(table.size()) + " values (" +
