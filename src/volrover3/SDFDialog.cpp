...
SDFDialog::SDFDialog(std::shared_ptr<SceneGraph> sceneGraph, QWidget *parent)
    ...
    , m_dimXSpinBox(nullptr)
    , m_dimYSpinBox(nullptr)
    , m_dimZSpinBox(nullptr)
    , m_algorithmComboBox(nullptr)
    , m_flipNormalsCheckBox(nullptr)
    , m_useBoundsCheckBox(nullptr)
    , m_minXSpinBox(nullptr)
    , m_minYSpinBox(nullptr)
    , m_minZSpinBox(nullptr)
    , m_maxXSpinBox(nullptr)
    , m_maxYSpinBox(nullptr)
    , m_maxZSpinBox(nullptr)
...
void SDFDialog::setupUI()
{
    ...
    m_useBoundsCheckBox = new QCheckBox(tr("Use custom bounding box (unchecked = use geometry extents)"), this);
...
void SDFDialog::connectSignals()
{
    connect(m_geometryComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &SDFDialog::onGeometrySelected);
...
void SDFDialog::onComputeClicked()
{
    ...
    if (m_geometryComboBox->currentIndex() < 0) {
        QMessageBox::warning(this, tr("No Geometry"),
                           tr("Please select a geometry to compute SDF."));
        return;
    }
    ...
    if (!geomNode) {
        QMessageBox::critical(this, tr("Error"),
                            tr("Failed to get geometry node."));
        return;
    }
    
    // Get parameters
    cvc::dimension dim(m_dimXSpinBox->value(), 
                       m_dimYSpinBox->value(), 
                       m_dimZSpinBox->value());
    
    cvc::bounding_box bbox;
    if (m_useBoundsCheckBox->isChecked()) {
        bbox = cvc::bounding_box(m_minXSpinBox->value(), m_minYSpinBox->value(), m_minZSpinBox->value(),
                                 m_maxXSpinBox->value(), m_maxYSpinBox->value(), m_maxZSpinBox->value());
    } else {
        const cvc::geometry* geomPtr = geomNode->getGeometry();
        if (!geomPtr) {
            QMessageBox::critical(this, tr("Error"),
                                tr("Failed to get geometry."));
            return;
        }
        bbox = geomPtr->extents();
    }
    
    cvc::sdf_algorithm algorithm = static_cast<cvc::sdf_algorithm>(
        m_algorithmComboBox->currentData().toInt());
        ...
        [this, geom, dim, bbox, algorithm, flipNormals, geomName, activeKey = m_activeThreadKey]() {
            ...
            try {
                ...
                QMetaObject::invokeMethod(this, [this]() {
                    m_statusLabel->setText(tr("Adding SDF volume to scene..."));
                }, Qt::QueuedConnection);
                ...
                QMetaObject::invokeMethod(this, [this]() {
                    onComputeFinished(false, "Computation cancelled");
                }, Qt::QueuedConnection);
            } catch (const std::exception& e) {
...
void SDFDialog::updateProgress(int value)
{
