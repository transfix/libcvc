...
IsosurfaceDialog::IsosurfaceDialog(std::shared_ptr<SceneGraph> sceneGraph, QWidget *parent)
    ...
    , m_improveIterationsSpinBox(nullptr)
    , m_normalTypeComboBox(nullptr)
    , m_computeButton(nullptr)
...
void IsosurfaceDialog::setupUI()
{
    ...
    m_improveIterationsSpinBox->setToolTip(tr("Number of mesh improvement iterations (0 = no improvement)"));
    paramLayout->addRow(tr("Improve Iterations:"), m_improveIterationsSpinBox);
    
    m_normalTypeComboBox = new QComboBox(this);
    m_normalTypeComboBox->addItem(tr("B-Spline Convolution"), static_cast<int>(cvc::BSPLINE_CONVOLUTION));
    m_normalTypeComboBox->addItem(tr("Central Difference"), static_cast<int>(cvc::CENTRAL_DIFFERENCE));
    m_normalTypeComboBox->addItem(tr("B-Spline Interpolation"), static_cast<int>(cvc::BSPLINE_INTERPOLATION));
...
void IsosurfaceDialog::connectSignals()
{
    connect(m_volumeComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &IsosurfaceDialog::onVolumeSelected);
        ...
        if (volumeNode && volumeNode->getState().fullName() == volumePath) {
            ...
            if (minVal.has_value() && maxVal.has_value()) {
                try {
                    ...
                    m_isovalueSpinBox->setRange(dataMin - (dataMax - dataMin), 
...
void IsosurfaceDialog::onComputeClicked()
{
    ...
    if (!volPtr) {
        QMessageBox::critical(this, tr("Error"),
                            tr("Volume has no data loaded."));
        return;
    }
    
    // Get parameters
    double isovalue = m_isovalueSpinBox->value();
    cvc::extraction_method method = static_cast<cvc::extraction_method>(
        m_methodComboBox->currentData().toInt());
    int improveIterations = m_improveIterationsSpinBox->value();
    cvc::normal_type normalType = static_cast<cvc::normal_type>(
        m_normalTypeComboBox->currentData().toInt());
        ...
        [this, vol, isovalue, method, improveIterations, normalType, volumeName]() {
            ...
            try {
                ...
                QMetaObject::invokeMethod(this, [this]() {
                    m_statusLabel->setText(tr("Adding isosurface to scene..."));
                }, Qt::QueuedConnection);
                ...
                m_sceneGraph->postEvent([this, isoGeom, volumeName, isovalue]() {
                    ...
                    try {
                        ...
                        QMetaObject::invokeMethod(this, [this]() {
                            onComputeFinished(true, "Isosurface extracted successfully");
                        }, Qt::QueuedConnection);
                ...
                QMetaObject::invokeMethod(this, [this]() {
                    onComputeFinished(false, "Extraction cancelled");
                }, Qt::QueuedConnection);
            } catch (const std::exception& e) {
...
void IsosurfaceDialog::updateProgress(int value)
{
