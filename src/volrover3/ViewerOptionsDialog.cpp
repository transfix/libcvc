...
ViewerOptionsDialog::ViewerOptionsDialog(VTKRenderWidget *renderWidget,
    ...
    , m_showFPSCheckBox(nullptr)
    , m_graphicsRootComboBox(nullptr)
    , m_refreshRootsButton(nullptr)
    , m_cameraComboBox(nullptr)
    , m_refreshCamerasButton(nullptr)
{
    setupUI();
    connectSignals();
    loadFromState();
...
void ViewerOptionsDialog::setupUI()
{
    ...
    m_showFPSCheckBox->setToolTip(tr("Display frames per second in the top-left corner of the viewer"));
    ...
    m_graphicsRootComboBox->setToolTip(tr("Select which graphics scene to render (for future multi-scene support)"));
...
{
    connect(m_showFPSCheckBox, &QCheckBox::toggled,
            this, &ViewerOptionsDialog::onShowFPSChanged);
    
    connect(m_graphicsRootComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &ViewerOptionsDialog::onGraphicsRootChanged);
    
    connect(m_cameraComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &ViewerOptionsDialog::onCameraChanged);
    
    connect(m_refreshRootsButton, &QPushButton::clicked,
            this, &ViewerOptionsDialog::refreshGraphicsRoots);
    
    connect(m_refreshCamerasButton, &QPushButton::clicked,
            this, &ViewerOptionsDialog::refreshCameras);
}
...
void ViewerOptionsDialog::closeEvent(QCloseEvent *event)
{
...
void ViewerOptionsDialog::refreshGraphicsRoots()
{
    ...
    if (m_sceneGraph) {
        ...
        // Currently only one graphics root is supported
        m_graphicsRootComboBox->addItem(
            QString::fromStdString(statePrefix + ".graphics.root"));
    }
...
void ViewerOptionsDialog::refreshCameras()
{
    ...
    if (m_sceneGraph) {
        std::string statePrefix = m_sceneGraph->getStatePrefix();
        // Currently only one camera is supported
        m_cameraComboBox->addItem(
            QString::fromStdString(statePrefix + ".camera"));
    }
