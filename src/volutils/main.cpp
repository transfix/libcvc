...
// Per-process cvc::app instance for volutils.  Created on first access.
// Using a function-local static avoids reliance on volutils_app() —
// the singleton we're removing — while still giving every subcommand a
// stable reference to pass to cvc::volume et al.
static cvc::app &volutils_app() {
  static cvc::app app;
  return app;
}
...
static int cmd_info(int argc, char **argv) {
  ...
  for (unsigned v = 0; v < vfi.numVariables(); ++v) {
    std::cout << "  Var " << v << ": type=" << vfi.voxelTypeStr(v) << " min=" << vfi.min(v, 0)
              << " max=" << vfi.max(v, 0) << "\n";
  }
  return 0;
}
...
static int cmd_add(int argc, char **argv) {
  ...
  auto &_app = volutils_app();
  cvc::volume a(_app), b(_app);
  a.read(inputs[0]);
  b.read(inputs[1]);
...
static int cmd_mask(int argc, char **argv) {
  ...
  auto &_app = volutils_app();
  cvc::volume vol(_app), mask_vol(_app);
  vol.read(vm["input"].as<std::string>());
...
static int cmd_backproject(int argc, char **argv) {
  po::options_description desc(
      "volutils backproject - filtered back-projection (tomographic reconstruction)");
  desc.add_options()("help,h", "show help")("input,i", po::value<std::string>()->required(),
...
static int cmd_rgba_merge(int argc, char **argv) {
  ...
  auto &_app2 = volutils_app();
  cvc::volume r(_app2), g(_app2), b(_app2), a(_app2);
  r.read(inputs[0]);
  g.read(inputs[1]);
  b.read(inputs[2]);
  a.read(inputs[3]);
