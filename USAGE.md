...
```bash
# Linux / macOS
tar xf libcvc-1.0.0-linux-x86_64-release.tar.gz   # → ./libcvc-1.0.0/
# or
unzip libcvc-1.0.0-macos-arm64-release.zip        # → ./libcvc-1.0.0/

# Windows (PowerShell)
Expand-Archive libcvc-1.0.0-windows-x86_64-release.zip
```
...
```bash
cmake -B build -DCMAKE_PREFIX_PATH=/path/to/libcvc-1.0.0
```

You can also set `cvc_DIR` directly:

```bash
cmake -B build -Dcvc_DIR=/path/to/libcvc-1.0.0/lib/cmake/cvc
```
