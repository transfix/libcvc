# cvcpkg/recipes/cvcgl-cuda/build.ps1 — build the cvcGL scene-graph library standalone
# against an installed libcvc SDK + VTK (Windows). Installs cvcGL.lib, the cvc/gl
# headers, and a cvcGLConfig.cmake so consumers can find_package(cvcGL CONFIG)
# and link cvc::cvcGL.
...
$args = @(
  '-G', 'Ninja',
  '-S', "$env:CVC_SOURCE_DIR/src/cvcGL",
  '-B', $env:CVC_BUILD_DIR,
