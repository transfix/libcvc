#!/usr/bin/env bash
# cvcpkg/recipes/cvc-cli-cuda/build.sh — build the `cvc` command-line tool standalone
# against an installed libcvc SDK + Boost.program_options. Compiles only
# src/cvc-cli (one translation unit) and links the pre-installed cvc::cvc, so
# libcvc is NOT rebuilt. Installs just the `cvc` executable into bin/.
set -euo pipefail
